...
**UPDATE (2026-08-04):** The custom derivation below was never finished — `hash`/`cargoHash` were left as
literal placeholders past implementation time, and that half-finished file is what eventually broke `nix
build` (a placeholder-hash mismatch in `fetchCargoVendor`, network-independent — it fails whether or not
crates.io is reachable). nixpkgs now packages `3axap4eHko/prmt` directly (0.7.0, prebuilt in the binary
cache), so `nix/pkgs/prmt.nix` was deleted and every reference switched to plain `pkgs.prmt`. The section
below is kept for historical record only — do not recreate this file from it.
...
| `nix/pkgs/prmt.nix`                    | Superseded — never finished (placeholder hashes), later deleted; `pkgs.prmt` from nixpkgs is used instead (see Nix Packaging update above) |
| `nix/modules/home/packages/shared.nix` | Uses plain `prmt` (nixpkgs), not a local `callPackage`                                                                                     |
| `nix/modules/nixos/core/programs.nix`  | Removed `programs.starship.enable = true`                                                                                                  |
| `nix/modules/home/dotfiles/shared.nix` | Removed `.config/starship.toml` entry                                                                                                      |
| `Makefile.d/install.mk`                | Removed `starship.toml .config/starship.toml` from `DOTFILES_MAP`                                                                          |

`starship.toml` was deleted from the repository (Task 5 of the plan), not kept as a reference — this note
originally said otherwise; corrected 2026-08-04.
