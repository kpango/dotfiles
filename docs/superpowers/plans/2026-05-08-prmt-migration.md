...
> **STATUS (2026-08-04): Tasks 2–5 complete, verified against the live repo. Task 1 as written below is
> superseded — do not execute it.** nixpkgs now packages `3axap4eHko/prmt` directly (0.7.0, prebuilt in the
> binary cache), so the custom derivation this task describes is unnecessary. It was in fact half-created —
> `nix/pkgs/prmt.nix` existed with literal `sha256-AAAA…` placeholders for both `src.hash` and `cargoHash`
> because Steps 2–8 were never run — and that half-finished file is what broke `nix build` for months (a
> `fetchCargoVendor` placeholder-hash mismatch, hit locally when building the darwin closure; nixpkgs'
> `lumen` had the identical failure mode and broke the `NixOS build` CI job on every run for the same reason).
> The fix removed `nix/pkgs/prmt.nix` and `nix/pkgs/lumen.nix` entirely and switched both to plain
> `pkgs.prmt` / `pkgs.lumen`. See `nix/overlays/default.nix` for the full account. If prmt is ever _not_ in
> nixpkgs again, re-derive Task 1 from scratch — do not resurrect this file's placeholder hashes.
...
### Task 1: Create nix/pkgs/prmt.nix — SUPERSEDED, see status note above. Do not execute.
...
### Task 2: Wire prmt into Nix — packages/shared.nix and nixos/core/programs.nix — done, verified 2026-08-04 (end state matches; checkboxes below were never ticked historically, left as-is rather than retrofitted)
...
### Task 3: Replace sheldon starship plugin with prmt — done, verified 2026-08-04
...
### Task 4: Remove starship.toml deployments — done, verified 2026-08-04
...
### Task 5: Remove starship from Docker volumes and delete starship.toml — done, verified 2026-08-04 (starship.toml is fully deleted, not merely undeployed — see spec status note)
