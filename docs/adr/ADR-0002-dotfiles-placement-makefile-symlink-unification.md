...
実機での `make nix/switch` 実行で、`DOTFILES_MAP` 全体を Make 側の唯一の真実源とする前提（§4「決定結果と根拠」1.）が一部の宛先では成立しないことも判明した。`nix/modules/home/programs/` 配下の4つの `programs.*` モジュール（zsh/git/tmux/helix）のうち3つ（`zsh.nix`/`git.nix`/`tmux.nix`）が単純ミラー以上のことをしていた: `programs.zsh`（`.zshrc`/`.zshenv`、ライブ checkout を `source` する home-manager 生成の合成ファイル）、`programs.git`（`.gitconfig`、`includes` でライブ checkout の `gitconfig` を読み込みつつ identity を上書き）、`programs.tmux`（`.tmux.conf`、TPM 行を除去してプラグインを宣言的に管理）。`darwin.nix` の `.gnupg/gpg-agent.conf`（Darwin のみ、Nix ビルド値埋め込み）と合わせて計4種類・5パスが対象（`helix.nix` は `programs.helix.enable = true` のみで設定ファイルを生成しないため対象外）。

`dotfiles/install` がこれらを毎回生の live-repo symlink で上書きすると、次の activation で home-manager の `checkLinkTargets` が「自分が置いたものではない」として拒否する（`zsh.nix` 自身のコメントが既に記録している通り、`force = true` は `.zshrc`/`.zshenv` のキー形状には効かない）。実際、`~/.gitconfig` がこの経緯で live checkout の `gitconfig` への symlink になっていたことで、ホスト上の `credential-leak-scan` フックインストーラが `~/.gitconfig` のつもりで書き込んだ `hooksPath` が、tracked な `gitconfig` ファイル自体に紛れ込む事故も発生した（別途 `git diff -- gitconfig` で検出、この ADR の変更には含めていない）。

`Makefile.d/install.mk` の `dotfiles/install` に `NIX_MANAGED=1` フラグを追加し、この5パス（`.zshrc`/`.zshenv`/`.gitconfig`/`.tmux.conf` は全 OS、`.gnupg/gpg-agent.conf` は Darwin のみ）を Nix 起動時だけスキップするようにした。genuine Arch Linux の素の bootstrap（`arch/install`/`mac/install` 初回）は `NIX_MANAGED` を渡さないため影響を受けない。

上記5パスの修正後、実機での `make nix/switch` はさらに先（`installCargoBinTools`/`installPackages`/`linkGeneration` を経て「orphan link のクリーンアップ」ステップ）まで進んだ後に別の失敗を起こした。home-manager 自身が生成する `activate` スクリプトが、そのクリーンアップ処理を `xargs -0 bash /nix/store/.../cleanup "$newGenFiles"` という**絶対パスを使わない裸の `bash`** で呼び出しており（home-manager 本体側のコード、このリポジトリの管理下にはない）、`home.packages` に nix 提供の `bash` が無いと PATH 解決が macOS 標準の `/bin/bash`（3.2、`home-manager.sh` の `setupColors` が使う `-v` テスト演算子が bash 4.2 以降必須）にフォールバックし、`conditional binary operator expected` で失敗することが判明した（`/bin/bash -c '[[ ! -v NO_COLOR ]]'` で再現確認済み）。`dotfilesAgentToolsInstall` の `export PATH=...` は同一シェルプロセス内でこの後続ステップまで引き継がれるため、`shared.nix` の `home.packages` に `bash` を追加するだけで解決した。
