...
description: >-
  ...
  差分を起案する。トリガー: 人間による /swarm-evolve の明示招集のみ、または `/loop <interval> /swarm-evolve`
  による定期起動。境界条件: **いかなる差分も人間の明示承認なしに適用しない**（docs-only であっても例外なし）。
  エージェント自身の行動規範ファイルを書き換えるという性質上、swarm-architect/swarm-release-gate と同格の
  最高警戒レベルで扱う。コード（vald/dotfiles のプロダクトコード）の変更はこの skill の対象外
  （swarm-implement を使う）。
...
- 証拠に基づかない改善提案（一般論としての「ベストプラクティス」）は禁止。**この skill は反復実証された
  パターンのみを対象にする**。
- 出力を `/tmp/claude-swarm/evolve-proposals/<日付>-draft.md` に Write する。
...
出力は `/tmp/claude-swarm/evolve-proposals/<日付>-verdict.md` に各提案の `approved: bool` と理由。
...
- 却下・保留分は `/tmp/claude-swarm/evolve-proposals/` に残し、次回 `/swarm-evolve` 実行時に再提示しない
  （却下理由をファイル名に含めて識別する）。
