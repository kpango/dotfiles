...
**対象 diff の準備**: `SWARM.md §2「Phase 4.5/G4.5 diff-supply プロトコル」`に従う。ミッションworktree内で
`git diff $(git merge-base HEAD <base-branch>)..HEAD` を取得し、一時ファイル
`/tmp/${CLAUDE_CODE_SESSION_ID:-manual}/swarm/<mission-slug>-diff.txt` へ直接リダイレクトで保存する
（オーケストレーター自身が書き込むため Agent 側に `Write` 権限は不要）。このファイルパスと変更ファイル数
（`--stat` の要約行から読み取る、追加の `git` 実行不要）を下記バッチ実行の**全 8 Agent の呼び出し prompt
へメタデータとして渡す**（`Bash` を持つ agent であっても、この対象diffについては自前で再取得させない —
トークンコストの重複を避けるのが目的）。コンテキストに収まらない規模なら、Phase 5 consistency-verify
と同じ取得法（`git diff --name-only <base>..HEAD`）で変更ファイル一覧のみを同様に一時ファイルへ保存し、
ファイルパスを渡す（各 Agent 自身に該当ファイルを `Read` させる）。
...
ツール実行に限定する（各 Agent 定義の本文指示による運用上の制約であり、Claude Code の `tools`
...
**実行順序（SWARM.md §1「最大 3 並列」、`swarm-parallel-gate.sh` が機械強制する既存制約にそのまま従う。
3 並列 × 3 バッチ）**:

| バッチ | Agent                                    |
| ------ | ---------------------------------------- |
| 1      | security, architecture, perf-simd        |
| 2      | code-quality, docs-comment, systems-lang |
| 3      | shell-config, infra-config               |

各 Agent 呼び出しの prompt に `[parallel-task:review-<agent短縮名>-<mission-slug>]` マーカーを含める
（`swarm-implement` の既存規約と同一機構、SWARM.md §1）。バッチ内の3件は同一メッセージにまとめて並列起動
する。バッチ完了ごとに、そのバッチで実際に起動した各 task-id（`review-<agent短縮名>-<mission-slug>`、
バッチ1・2は3件、バッチ3は2件）についてそれぞれ1回ずつ
`~/.claude/skills/swarm-implement/scripts/parallel-gate.sh --release <task-id>` を呼び、使用した全スロットを
解放してから次バッチへ進む（1回のみの呼び出しでは1スロットしか解放されない。解放しないとTTL 1800s経過
まで次バッチがブロックされる）。
