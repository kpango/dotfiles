...
4. 承認・統合後（**順序規定**: 自分でコミットを作る統合〈Option 1 のローカルマージ〉では `AGENTS.md` 軌跡追記と
   `self-improve-register.sh` 実行をコミット**前**にミッション本体と同一コミットへ含める — 後追い chore
   コミットでの登録漏れ再発防止）: `AGENTS.md` へ軌跡追記（2 回目の学びは機械化チェックへ昇格済みか確認）→
   **自己改善ミッション（`self-improve-targets` を指定して `mission-init.sh` した場合のみ）は**
   ...
   ミッション予算カウンタ掃除（`budget-guard.sh --reset <task-id> --mission=<slug>` — Tasks テーブルの
   **全 task-id** について `--reset` を繰り返し実行する（1 タスク = 1 task-id のため単一リセットでは
   済まない）。`--mission` を忘れると共有ミッションカウンタと fable ミッションカウンタが残存する。
   fable カウンタ・未消費 grant も掃除されるが、`fable-spot-log.jsonl` は観測記録のため削除しない）→
...
`AGENTS.md`/`@fix_plan.md`（Phase 5 参照）は**プロジェクト単位**のミッション軌跡であり、個々のタスクの
...
得られた場合のみ追記する。プロジェクト固有の学びは引き続き `AGENTS.md` へ、本 skill 自体の運用知見のみ
