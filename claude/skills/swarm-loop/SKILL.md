...
1. 人間へ `/swarm-release-gate` の招集を要請する（自分でマージしない）。決定論的検証（`verify.sh`）が全パス
   ...
   **フォールバック**: 人間が `/swarm-release-gate` を入力せず「続けて」等で応答した場合は、
   ...
   できないため。gate の本質は人間の最終判断であり skill 起動の儀式ではない）。
...
4. 承認・統合後（**順序規定**: 自分でコミットを作る統合〈Option 1 のローカルマージ・main 直接コミット運用〉
   では、`AGENTS.md` への軌跡追記と `self-improve-register.sh` の実行をコミット**前**に行い、ミッション本体と
   同一コミットに含める — 登録が後追いの chore コミットになる漏れが 2 ミッション連続で発生したための順序の
   機械化）: `AGENTS.md` へ軌跡追記（2 回目の学びは機械化チェックへ昇格済みか確認）→
   **自己改善ミッション（`self-improve-targets` を指定して `mission-init.sh` した場合のみ）は**
   ...
   再実行しても重複行は作らない）→**
   `Skill(swarm-memory-sync)` を、今回 `AGENTS.md` に追記した行 + `@fix_plan.md` の
   ...
   ミッション予算カウンタ掃除（`budget-guard.sh --reset <task-id> --mission=<slug>` — `--mission` を
   忘れると共有ミッションカウンタと fable ミッションカウンタが残存する。fable カウンタ・未消費 grant も
   掃除されるが、`fable-spot-log.jsonl` は観測記録のため削除しない）→ `@fix_plan.md` を削除またはアーカイブ →
   最終レポート（done/blocked 一覧・学び・残課題・auto-memory 反映件数）を提示して終了。
5. **Quick モードの完了処理**: `@fix_plan.md` が無いため、テスト全通過を確認し、変更要約を提示して終了。
   ワークツリークリーンアップ・軌跡追記は変更が非自明だった場合のみ行う。Quick は規模上 `swarm-evolve`
   自動証跡収集(上記 2.)も行わない（1 ファイル 15 行以下の変更が対象のため証跡が薄く、コスト対効果が低い）。
