...
## 7.5 Phase G4.5: ADVERSARIAL REVIEW（8 種専門 agent・3 並列×3 バッチ）

**発火点**: 直前の ESCALATE 行列で「`ready` が空 かつ 全ノード done」と判定された時点。Phase G5 GATE へ
進む前に必ず本 Phase を通す。

**対象**: `SWARM.md §2「Phase 4.5/G4.5 diff-supply プロトコル」`に従う。ミッションworktreeの base-ref
から、`@fix_plan.md` の現行 Tasks テーブルで `done` の全ノードのブランチ diff を集約したもの。file→node
対応表を作った上で各ノードの `git diff <base>..<branch>` を連結して 1 つの review artifact とする。この
review artifact を一時ファイル `/tmp/${CLAUDE_CODE_SESSION_ID:-manual}/swarm/<mission-slug>-diff.txt`
（§9 既存不変条件）へ直接リダイレクトで保存する（オーケストレーター自身が書き込むため Agent 側に
`Write` 権限は不要）。このファイルパスと変更ファイル数（連結対象の各ノードdiffの`+++ b/`行数の合計、
追加の`git`実行不要）を下記バッチ実行の**全 8 agent の呼び出し prompt へメタデータとして渡す**（`Bash`
を持つ agent であっても、この対象diffについては自前で再取得させない — トークンコストの重複を避ける
のが目的。swarm-loop側〈Phase 4.5〉と同一方針）。コンテキストに収まらない規模なら、swarm-loop側
〈Phase 4.5〉と同じ取得法（`git diff --name-only <base>..<branch>`を全ノード分連結）で変更ファイル
一覧のみを同様に一時ファイルへ保存し、ファイルパスを渡す（各agent自身に該当ファイルを`Read`させる）。

**8 agent（全 `model: sonnet` 明示。4体は `tools: Read, Grep, Glob` のみ — 供給されたファイルパスを
`Read` で読む。残り4体〈security/perf-simd/shell-config/infra-config〉は追加で `Bash` を持つ —
security/perf-simd は供給されたファイルパスを `Bash` でクロスファイルパターン検索に使い、shell-config/
infra-config は `Bash` を `git log`/`git blame` 等の補助調査、または shellcheck/shfmt 等の静的解析
ツール実行に限定する。Claude Code の `tools` frontmatterはコマンド単位の制限を提供しないため
「読み取り専用」は本文指示による運用上の制約であり機構的な強制ではない。信頼境界・完全性の自己検証・
最小権限の詳細は SWARM.md §2 参照）を 3 並列×3 バッチで実行する**（SWARM.md §1「最大 3 並列」の既存機械強制にそのまま
従う。各 Agent 呼び出しの prompt に `[parallel-task:review-<agent短縮名>-<mission-slug>]` マーカーを
含める（swarm-loop側〈Phase 4.5〉と同一形式に統一 — mission-slugを含めることで並行ミッション間の
task-id衝突を避ける）。バッチ完了ごとに、そのバッチで実際に起動した各 `<agent短縮名>`（バッチ1・2は
3件、バッチ3は2件）についてそれぞれ1回ずつ
`~/.claude/skills/swarm-implement/scripts/parallel-gate.sh --release review-<agent短縮名>-<mission-slug>`
を呼び、使用した全スロットを解放してから次バッチへ進む（1回のみの呼び出しでは1スロットしか解放
されない。解放しないとTTL 1800s経過まで次バッチがブロックされる）:

| バッチ | agent                               | 観点                             | 既存 agent との関係                                        |
| ------ | ----------------------------------- | -------------------------------- | ---------------------------------------------------------- |
| 1      | `security-adversarial-reviewer`     | セキュリティ                     | `security-audit` との二段階防御                            |
| 1      | `architecture-adversarial-reviewer` | アーキテクチャ整合性             | 新規領域                                                   |
| 1      | `perf-simd-adversarial-reviewer`    | 性能・SIMD                       | `perf-analyzer`／`ann-perf-engineer` との二段階防御        |
| 2      | `code-quality-adversarial-reviewer` | コード品質                       | `code-reviewer` との二段階防御                             |
| 2      | `docs-comment-adversarial-reviewer` | 技術文書・コメント品質           | 新規領域                                                   |
| 2      | `systems-lang-adversarial-reviewer` | Go/Rust/C++ 言語仕様             | `go-expert`／`rust-expert`／`code-reviewer` との二段階防御 |
| 3      | `shell-config-adversarial-reviewer` | Shell/ZSH/Makefile 言語仕様      | 新規領域                                                   |
| 3      | `infra-config-adversarial-reviewer` | Nix/Lua/Yaml/Json 構文・スキーマ | 新規領域（`vald-reviewer` とは非重複）                     |

**判定と FAIL 時のルーティング**:

- 各 Agent の応答末尾の `VERDICT: PASS | FAIL` 行を読む。欠落・停滞時はSWARM.md §2「名前付きAgent
  呼び出しの停滞対応」に従う。
- 8 agent全員`VERDICT: PASS` → Phase G5 GATEへ進む。
