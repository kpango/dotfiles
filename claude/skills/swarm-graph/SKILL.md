...
<slug> "<goal>" <interactive|mission> [self-improve-targets]` を再利用する（scale 判定基準は
  ...
  追記する（graph ミッションのマーカー）。生成される `@fix_plan.md` の `## Invariants` 節
  （`mission-init.sh` 共有、ConstraintRot対策 arXiv:2606.22528）は `swarm-loop` と同一。
...
- **`budget-guard --reset` を含む全ノードの予算カウンタ掃除は
  ...
  変わらないが、スクリプトが Tasks 節から全 task-id を自動抽出する）。worktree回収は
