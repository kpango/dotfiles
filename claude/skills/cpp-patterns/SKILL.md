...
| SOLID                       | C++ 実装                                                                   | 関連ガイドライン |
| --------------------------- | -------------------------------------------------------------------------- | ---------------- |
| **S** Single Responsibility | 1クラス = 1変更理由; メンバは同一の責務に属する                            | F.2              |
| **O** Open/Closed           | `virtual`/`override` + テンプレート/concept で拡張; 基底クラスを変更しない | C.128, T.1       |
| **L** Liskov Substitution   | 派生クラスは基底の事前・事後条件を強化/弱化しない                          | C.35, C.128      |
| **I** Interface Segregation | 純粋仮想関数のみの薄いインターフェースクラスを分離する                     | C.9, I.1         |
| **D** Dependency Inversion  | 具体型ではなく抽象クラス/concept に依存する                                | T.10, I.4        |
