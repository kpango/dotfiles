...
| ------------------------- | -------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `dig`                     | `/dig`                     | ワークスペース隔離→コードベース分析→設計提案→実装計画→自律実行の統合ワークフロー。汎用開発手法はすべてdigで完走 |
...
```
/dig <目標>                    # コードを分析し、設計提案し、計画し、実装まで完走
/dig                           # 目標未指定の場合、ヒアリングからスタート
```

5フェーズで完走: Phase 0 (worktree隔離) → Phase 1 (codegraph/graphify分析) → Phase 2 (設計インタビュー+提案) → Phase 3 (TDAD実装計画) → Phase 4 (体系的デバッグ・コードレビュー・ブランチ完了まで自律実行)
