...
| `graphify`      | ナレッジグラフ CLI                               | zsh 関数ラッパー経由（`pass show ai/agy` から API キー取得、`pass` に `ai/agy` エントリが必要） |
...
`pass show ai/agy` を自身の API キー取得手段に置き換えること。
...
| `systems-lang-adversarial-reviewer` | Adversarial Go/Rust/C++ language-spec review — second line of defense with `go-expert`/`rust-expert`/`code-reviewer` | sonnet                |
| `shell-config-adversarial-reviewer` | Adversarial Shell/Zsh/Makefile language-spec review — new coverage area                                              | sonnet                |
| `infra-config-adversarial-reviewer` | Adversarial Nix/Lua/YAML/JSON syntax & schema review — new coverage area, non-overlapping with `vald-reviewer`       | sonnet                |

Of the 8 `*-adversarial-reviewer` agents above, 4 (`security`/`perf-simd`/`code-quality`/`systems-lang`) are the second stage of a two-stage defense (existing `code-reviewer`/`security-audit`/`perf-analyzer`/`ann-perf-engineer`/`go-expert`/`rust-expert` remain the first stage for ordinary in-progress review); the other 4 (`architecture`/`docs-comment`/`shell-config`/`infra-config`) cover new territory with no existing first-stage counterpart. All 8 are invoked automatically by `/swarm-loop`/`/swarm-graph` immediately before the Phase 5/G5 GATE and are not intended for manual `@-mention` or auto-delegation like the rest of this table.
...
| swarm-loop/swarm-graph Phase 5/G5 GATE — adversarial security re-review (orchestrator auto-invokes, not manual)                      | `security-adversarial-reviewer`     |
| swarm-loop/swarm-graph Phase 5/G5 GATE — adversarial architecture-consistency review (orchestrator auto-invokes, not manual)         | `architecture-adversarial-reviewer` |
| swarm-loop/swarm-graph Phase 5/G5 GATE — adversarial perf/SIMD re-review (orchestrator auto-invokes, not manual)                     | `perf-simd-adversarial-reviewer`    |
| swarm-loop/swarm-graph Phase 5/G5 GATE — adversarial code-quality re-review (orchestrator auto-invokes, not manual)                  | `code-quality-adversarial-reviewer` |
| swarm-loop/swarm-graph Phase 5/G5 GATE — adversarial docs/comment quality review (orchestrator auto-invokes, not manual)             | `docs-comment-adversarial-reviewer` |
| swarm-loop/swarm-graph Phase 5/G5 GATE — adversarial Go/Rust/C++ language-spec review (orchestrator auto-invokes, not manual)        | `systems-lang-adversarial-reviewer` |
| swarm-loop/swarm-graph Phase 5/G5 GATE — adversarial Shell/Zsh/Makefile language-spec review (orchestrator auto-invokes, not manual) | `shell-config-adversarial-reviewer` |
| swarm-loop/swarm-graph Phase 5/G5 GATE — adversarial Nix/Lua/YAML/JSON syntax/schema review (orchestrator auto-invokes, not manual)  | `infra-config-adversarial-reviewer` |
...
- The 8 `*-adversarial-reviewer` agents are for the GATE-immediate adversarial re-review stage only — use the existing `code-reviewer`/`security-audit`/`perf-analyzer`/`ann-perf-engineer`/`go-expert`/`rust-expert` for ordinary in-progress review
...
| `swarm-relay`             | `/swarm-relay`、または swarm-loop/graph内部から参照 | cross-session messaging(ListAgents/SendMessage、独立した別セッション間)によるミッション間連携プロトコル定義＋スタンドアロン確認ユーティリティ。並行ミッションのgit index/HEAD競合の早期検知・クロスリポジトリ(vald⇔dotfiles)の学び伝播。Agent Teamsとは別機能(不採用済み、SWARM_REFERENCES.md参照)  |
