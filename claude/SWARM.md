...
「独立した角度からの攻撃 + 敵対的反証による収束」を持つ（confidence=medium・2-1 vote、詳細は §8 の
2026-08-01 追加分を参照）。本規約の Maker/Checker 分離・秘書集約はこの機能上に構築されているが、
...
- **Maker (Sonnet) と Checker (Opus) は同一ベンダー系列 (intra-family)**である。verifier の独立性研究では、
  ...
  - **hooks も「necessary but insufficient」である**: Anthropic 公式ガイダンス（2026-06）は hooks を
    ...
    指摘されている（GitHub RFC issue、confidence=medium・3-0 vote、詳細は §8 の 2026-08-01 追加分を
    参照）。本基盤にも同種の限界の実例がある: `swarm-fable-gate.sh` は `subagent_type: debugger` を
  ...
  - **既知の未機械化ギャップ（2026-08-03 決定論的検証カバレッジレンズ探索、7 Haiku shard、最重要項目は
    6/7 一致で確認）**: 本節冒頭の「両者が食い違う場合は hook 側を優先し、Checker には理由を再提示させる」
    という規範は、現状オーケストレーター（呼び出し元 LLM）自身の判断に委ねられており、矛盾状態（hook
    失敗 かつ Checker 合格）を自動検出して Checker 判定を無効化する専用の機械的チェックは存在しない。
    同種の未機械化ギャップとして、Maker/Checker 並列数上限（§1「最大 3 並列・4 並列以上禁止」）・テスト
    アサーション弱化検出（§6「データの完全性」）・3 試行目ソフトチェックポイントの自動判定（§3）・
    `swarm-implement` の Checker 判定行（`VERDICT: PASS|FAIL`）欠落検出（呼び出し元 LLM 自身の
    grep 実行に依存する案を検討したが、それ自体が「LLM が実行するかどうかに依存する」という点で
    prompt 強制と同じ enforcement カテゴリに留まり、2 ミッションで実証済みの再発を防げないため不採用
    ——真の機械化には `PostToolUse:Agent` 相当の hook 新設が必要）・`swarm-evolve` の Drafter/Checker
    独立性の実装的隔離（本文はいずれも prompt 指示のみで機械強制無し、上記とは別課題）がある。いずれも
    解決には専用の設計判断（hook での状態記録・比較機構・新規 hook 種別の追加等）が必要であり、次回の
    `/swarm-architect` 設計レビュー候補として記録するに留める（`swarm-evolve` の prune-capable 受理
    ゲート対応は 2026-08-03 に `/swarm-architect` へ設計レビューを提出済み・提案書のみで実装は別ミッション
    に引き渡し予定であり、上記ギャップ群とは別トラック）。
...
- コンテキスト管理は 2026-08 時点で最も強く裏付けられたテーマ（4 独立系・confidence=high）: Anthropic の
  ...
  `AGENTS.md` 設計と一致する（新規採用ではなく既存設計の裏付け）。Anthropic の context-management API は
...
- Claude Code の「Dynamic Workflows」機能（=本基盤で使用中の Workflow ツール）が Anthropic 公式に
  ...
  検証パターンは「独立 angle からの攻撃 + 敵対的反証で収束」。confidence=medium（2-1 vote）。§0 参照。
...
- Phantom Guardrails（arXiv:2607.13083）: naive add-only self-improving harness で LLM proposer が
  ...
  という結論。`swarm-evolve` の人間承認必須原則（§5）を補強する新エビデンス。confidence=medium
  （単一 preprint、独立再現なし）。
