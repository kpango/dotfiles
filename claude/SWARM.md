...
EXPLORE → PLAN(+設計インタビュー) → EXECUTE → CHECKPOINT → ADVERSARIAL REVIEW → GATE の状態機械で全層を
...
"fork"` 以外）が停滞することがある。この場合は**再スポーンせず** `SendMessage` で当該エージェントへ
  ...
  Fixer/Test Maker）・`swarm-evolve`（Drafter/Checker）・`swarm-loop`/`swarm-graph`（Phase 4.5/G4.5 の
  8 Agent 敵対的レビュー）が名前付き Agent を起動する全 skill であり、いずれもこの原則を適用する
  （詳細な運用例は `swarm-implement/SKILL.md` 参照）。
