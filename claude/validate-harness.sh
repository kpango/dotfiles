...
if [[ -f "$GHINT_FILE" && -x "$GHINT_FILE" ]]; then
    ...
    if [[ -f "$DOTFILES_DIR/.claude/graph/graphify/graph.json" ]]; then
        ...
        check "graphify-hint: valid JSON for grep + graph present" "WARN:no .claude/graph/graphify/graph.json in dotfiles"
    fi
else
