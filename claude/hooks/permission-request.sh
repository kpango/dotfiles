...
allow() {
    jq -n --arg reason "$1" '{"decision": "allow", "reason": $reason}'
    echo "${TIMESTAMP} permission-request: tool=${TOOL} rule=${RULE} outcome=allow" >> "${LOG_DIR}/sessions.log"
    exit 0
}
...
echo "${TIMESTAMP} permission-request: tool=${TOOL} rule=${RULE} outcome=passthrough" >> "${LOG_DIR}/sessions.log"
exit 0
