...
case "$file" in
  *.Dockerfile | */Dockerfile)
    ...
    ;;
  *.go)
    # vald のみ: 編集したパッケージに限定して golangci-lint を即時実行。ディレクトリ名の部分一致では
    # vald を別名でクローンした場合にサイレントに検証が無効化されるため、go.mod のモジュール名で判定する。
    grep -qm1 '^module github\.com/vdaas/vald$' "$root/go.mod" 2>/dev/null || exit 0
