...
# vald: 編集した Go パッケージに限定した golangci-lint。ディレクトリ名の部分一致では vald を別名で
# クローンした場合にサイレントに検証が無効化されるため、go.mod のモジュール名で判定する。
if grep -qm1 '^module github\.com/vdaas/vald$' "$root/go.mod" 2>/dev/null; then
  if [ -n "$go_dirs" ] && command -v golangci-lint >/dev/null 2>&1; then
    for d in $go_dirs; do
      ...
      # "build constraints exclude all Go files" without the tag; retry
      # with the tags extracted from the package's go:build lines before
      # reporting failure.
      if [ "$rc" -ne 0 ] && printf '%s' "$out" | grep -q "build constraints exclude all Go files"; then
        tags=$(grep -rhoP '(?<=^//go:build )[a-zA-Z0-9_]+$' "$d"/*.go 2>/dev/null | sort -u | paste -sd, -)
        if [ -n "$tags" ]; then
          out=$(cd "$root" && timeout 180 golangci-lint run --build-tags "$tags" --new-from-rev=HEAD "./$rel/" 2>&1)
          rc=$?
        fi
      fi
      [ "$rc" -ne 0 ] && [ "$rc" -ne 124 ] \
