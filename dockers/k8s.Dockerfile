...
set -x && cd "$(mktemp -d)" \
    ...
    && curl --retry ${CURL_RETRY} --retry-all-errors --retry-delay ${CURL_RETRY_DELAY} -fsSLGH "${HEADER}" "${RAWGITHUB}/helm/helm/main/scripts/get-helm-3" | bash \
    && unset HEADER \
    && BIN_NAME="helm" \
    && chmod a+x "${BIN_PATH}/${BIN_NAME}" \
    && upx -9 "${BIN_PATH}/${BIN_NAME}"
...
RUN set -x; cd "$(mktemp -d)" \
    && BIN_NAME="linkerd" \
    && curl --retry ${CURL_RETRY} --retry-all-errors --retry-delay ${CURL_RETRY_DELAY} -fsSL https://run.linkerd.io/install | sh \
...
set -x && cd "$(mktemp -d)" \
    ...
    && curl --retry ${CURL_RETRY} --retry-all-errors --retry-delay ${CURL_RETRY_DELAY} -fsSL https://istio.io/downloadIstio | ISTIO_VERSION="${VERSION}" sh - \
