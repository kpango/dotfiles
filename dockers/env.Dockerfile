...
# ubuntu:devel (base.Dockerfile's FROM) ships a default non-root "ubuntu" user/group at
# uid/gid 1000 — the same id ${USER}/${GROUP_ID} claim below via --non-unique. Left in place,
# both names resolve to the same numeric id in /etc/passwd, and getpwuid()-based lookups
# (`whoami`, `id`, prompts, git author auto-detection, ...) return whichever name sorts first
# in the file — "ubuntu", not "${WHOAMI}" — even though $HOME/${USER} etc. are otherwise
# correct. Remove it first so the id is unambiguously ${USER}'s. `|| true`: both must be
# no-ops (not build failures) if a future base image drops this user already.
RUN userdel -f -r ubuntu 2>/dev/null || true \
    && groupdel ubuntu 2>/dev/null || true \
    && groupadd --non-unique --gid ${GROUP_ID} docker \
