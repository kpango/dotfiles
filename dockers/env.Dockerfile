...
RUN --mount=type=secret,id=gat set -x && cd "$(mktemp -d)" \
    ...
    && curl --retry ${CURL_RETRY} --retry-all-errors --retry-delay ${CURL_RETRY_DELAY} -fsSLo "/tmp/${BIN_NAME}.tar.xz" "https://${REPO_NAME}.org/download/${VERSION}/${TAR_NAME}.tar.xz" \
...
RUN --mount=type=secret,id=gat set -x && cd "$(mktemp -d)" \
    ...
    && curl --retry ${CURL_RETRY} --retry-all-errors --retry-delay ${CURL_RETRY_DELAY} -fsSLo "./${BIN_NAME}.tar.xz" "${GITHUB}/${REPO}/${RELEASE_DL}/${VERSION}/${TAR_NAME}.tar.xz" \
