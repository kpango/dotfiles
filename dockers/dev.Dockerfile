...
RUN --mount=type=bind,source=gitattributes,target=/tmp/gitattributes,ro \
    ...
    --mount=type=bind,source=agy,target=/tmp/dotfiles/agy,ro \
    --mount=type=bind,source=pi,target=/tmp/dotfiles/pi,ro \
    ...
    install -d -m 755 -o "${USER_ID}" -g "${GROUP_ID}" \
        ...
        "${HOME}/.zcache" \
        "${HELIX_HOME}" \
    && make -C /tmp/dotfiles claude/docker/install pi/docker/install agy/docker/install ROOTDIR=/tmp/dotfiles HOME="${HOME}" USER_ID="${USER_ID}" GROUP_ID="${GROUP_ID}" \
