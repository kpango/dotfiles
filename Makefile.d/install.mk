...
claude/install: dotfiles/install
	...
	sudo mkdir -p $(ROOT_HOME)/.gemini/policies
	sudo ln -sfvn "$(ROOTDIR)/gemini/settings.json"        $(ROOT_HOME)/.gemini/settings.json
	sudo ln -sfvn "$(ROOTDIR)/gemini/policies/policy.toml" $(ROOT_HOME)/.gemini/policies/rules.toml
...
mac/install: dotfiles/install claude/install
	...
	for agent in $(MACOS_LAUNCH_AGENTS); do \
		sudo rm -f "$(HOME)/Library/LaunchAgents/$$agent"; \
		sudo cp -f "$(ROOTDIR)/macos/$$agent" "$(HOME)/Library/LaunchAgents/$$agent"; \
