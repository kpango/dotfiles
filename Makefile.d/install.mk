.PHONY: dotfiles/install dotfiles/compile dotfiles/clean dotfiles/perm precompile/zsh \
        ...
        mac/install m1/install m1air/install m3/install \
...
claude/SWARM.md .claude/SWARM.md
claude/SWARM_REFERENCES.md .claude/SWARM_REFERENCES.md
...
claude/docker/install:
	...
	install -m 644 -o "$(USER_ID)" -g "$(GROUP_ID)" \
		"$(ROOTDIR)/claude/SWARM_REFERENCES.md" "$(HOME)/.claude/SWARM_REFERENCES.md"
	install -m 755 -o "$(USER_ID)" -g "$(GROUP_ID)" \
		"$(ROOTDIR)/claude/statusline-command.sh" "$(HOME)/.claude/statusline-command.sh"
	find "$(ROOTDIR)/claude/rules" -maxdepth 1 -name "*.md" \
...
claude/install: dotfiles/install
	...
	ln -sfvn "$(ROOTDIR)/claude/rules"  "$(HOME)/.claude/rules"
	...
	sudo rm -rf $(ROOT_HOME)/.claude
	sudo ln -sfvn "$(HOME)/.claude" $(ROOT_HOME)/.claude
	sudo ln -sfvn "$(ROOTDIR)/gitconfig"                   $(ROOT_HOME)/.gitconfig
	sudo mkdir -p $(ROOT_HOME)/.agy/policies
	sudo ln -sfvn "$(ROOTDIR)/agy/settings.json"        $(ROOT_HOME)/.agy/settings.json
	sudo ln -sfvn "$(ROOTDIR)/agy/policies/policy.toml" $(ROOT_HOME)/.agy/policies/rules.toml
...
## deploy network configs (sysctl, dnsmasq, NM dispatcher, irqbalance) and restart services
## Idempotent: safe to run repeatedly without side effects.
## DNS note: after first run, run `tailscale set --accept-dns=false` to hand DNS control to dnsmasq.
network/install:
	sudo mkdir -p /etc/NetworkManager/dnsmasq.d /etc/sysctl.d
	@$(call DEPLOY_FUNC,$(ROOTDIR)/network/dns/dnsmasq.conf,/etc/NetworkManager/dnsmasq.d/dnsmasq.conf,sudo)
	...
	@echo "network/install: done. Run 'tailscale set --accept-dns=false' if not already done."
...
mac/install: dotfiles/install claude/install
	$(MAC_PREP)
