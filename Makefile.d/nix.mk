.PHONY: nix/setup \
	...
	nix/update nix/switch nix/pull
...
define NIX_SETUP_DARWIN
	...
	$$(command -v nix) run github:joshryandavis/defaults2nix -- -all -filter dates,state,uuids -out ./all-defaults.nix; \
...
# ─────────────────────────────────────────────────────────────────────────────
# nix/switch — rebuild and activate the running host's nix-darwin configuration
# ─────────────────────────────────────────────────────────────────────────────

# Delegates to the `nix-update` command installed by
# nix/modules/home/programs/nix-update.nix rather than running
# `sudo darwin-rebuild switch --flake` directly — that form fails here because
# darwin-rebuild re-execs as root, and nix's git fetcher then refuses to read
# this repo (owned by the login user). See that file's header comment for the
# full story. Prompts for a sudo password interactively.
## rebuild + activate this host's nix-darwin config (darwin only; prompts for sudo)
nix/switch:
	@if [ "$$(uname -s)" != "Darwin" ]; then \
		echo "Error: nix/switch only supports Darwin (macOS)."; \
		echo "       On NixOS, use: sudo nixos-rebuild switch --flake ./nix#$(NIX_HOST_NAME)"; \
		exit 1; \
	fi
	@if ! command -v nix-update >/dev/null 2>&1; then \
		echo "Error: 'nix-update' not found in PATH."; \
		echo "       It's installed by home-manager (nix/modules/home/programs/nix-update.nix)."; \
		echo "       Run 'make nix/setup' first to bootstrap the initial generation."; \
		exit 1; \
	fi
	@nix-update
...
# $(call _nix_eval_hosts, label, hosts-var-name, config-attr-prefix, done-msg)
define _nix_eval_hosts
@echo "==================================================="; \
echo " $(1)  — evaluating $(words $($(2))) host(s)"; \
echo "==================================================="; \
failed=0; \
for host in $($(2)); do \
	printf "  %-30s " "$$host"; \
	if $(NIX_RUN) eval \
		".#$(3).$$host.config.system.stateVersion" \
...
nix/test/eval:
	$(call _nix_eval_hosts,nix/test/eval,NIX_TEST_HOSTS,nixosConfigurations,All hosts evaluated successfully.)
...
nix/test/eval/darwin:
	$(call _nix_eval_hosts,nix/test/eval/darwin,NIX_TEST_DARWIN_HOSTS,darwinConfigurations,All darwin hosts evaluated successfully.)
