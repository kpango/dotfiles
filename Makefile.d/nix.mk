...
NIX_TEST_HOSTS ?= tr thinkpad-p1-gen5 thinkpad-x1-gen9 hp-dragonfly-g2
...
# Flags for test/check targets: never update flake.lock accidentally
NIX_FLAGS ?= --no-write-lock-file --show-trace \
...
## Override host with: make nix/test/build NIX_HOST_NAME=thinkpad-p1-gen5
nix/test/build:
	@echo "==================================================="
...
##
## Override host with: make nix/test/vm NIX_HOST_NAME=thinkpad-x1-gen9
nix/test/vm:
	@echo "==================================================="
...
## Override host with: make nix/test NIX_HOST_NAME=thinkpad-p1-gen5
...
nix/fmt/check:
	...
	@$(NIX_RUN) run nixpkgs#nixpkgs-fmt -- --check "$(ROOTDIR)/nix" $(NIX_FLAGS)

## Auto-format all .nix files with nixpkgs-fmt.
nix/fmt:
	@echo "=> Formatting Nix files..."
	@NIX_WRITABLE=1 $(NIX_RUN) run nixpkgs#nixpkgs-fmt -- "$(ROOTDIR)/nix" $(NIX_FLAGS)
