...
define NIX_SETUP_DARWIN
	if [ ! -f /Applications/Container.app/Contents/MacOS/Container ] && [ ! -d /Applications/Container.app ]; then \
		...
		if ! curl --fail --retry 3 --retry-all-errors --retry-delay 3 -LO https://github.com/apple/container/releases/download/0.4.1/container-0.4.1-installer-signed.pkg; then \
			echo "Error: failed to download the Apple Container installer (network error or the pinned 0.4.1 release/URL is gone)." >&2; \
			rm -f container-0.4.1-installer-signed.pkg; \
			exit 1; \
		fi; \
		if ! sudo installer -pkg container-0.4.1-installer-signed.pkg -target /; then \
			echo "Error: Apple Container installer (pkg) failed." >&2; \
			rm -f container-0.4.1-installer-signed.pkg; \
			exit 1; \
		fi; \
		rm -f container-0.4.1-installer-signed.pkg; \
