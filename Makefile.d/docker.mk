.PHONY: docker/build docker/build/prod docker/build/image docker/build/dev \
        ...
        docker/merge docker/merge/% docker/login docker/push docker/pull docker/profile \
...
docker/build/image:
	...
	@umask 077 && printf '%s' "$${GITHUB_ACCESS_TOKEN:-$$(pass github.api.ro.token 2>/dev/null)}" > $(TMP_DIR)/gat
...
docker/profile:
	rm -f analyze.txt
	command -v dlayer >/dev/null 2>&1 && docker save kpango/dev:$(VERSION) | dlayer >> analyze.txt
