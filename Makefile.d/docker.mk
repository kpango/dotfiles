...
docker/build/prod: \
	...
	docker/build/base
	@if command -v xpanes >/dev/null 2>&1; then \
		xpanes -s -c "$(MAKE) DOCKER_EXTRA_OPTS=\"--no-cache\" -f $(ROOTDIR)/Makefile docker/build/{}" go docker rust dart k8s nim gcloud zig nix env vald; \
	else \
		for img in go docker rust dart k8s nim gcloud zig nix env vald; do \
			$(MAKE) DOCKER_EXTRA_OPTS="--no-cache" -f $(ROOTDIR)/Makefile docker/build/$$img & \
		done; wait; \
	fi
