...
## run all formatters: zsh, nix, yaml, go, rust, json, md, makefile, conf
format: \
	format/zsh \
	format/dockerfile \
	format/nix \
...
## format zsh/bash scripts with shfmt (falls back to beautysh)
format/zsh:
	@if [ -n "$(FORMAT_ZSH_FILES)" ]; then \
		echo "Formatting zsh files..."; \
		if command -v shfmt >/dev/null 2>&1; then \
			shfmt -ln bash -w $(FORMAT_ZSH_FILES); \
			for _f in $(FORMAT_ZSH_FILES); do $(SED_INPLACE) 's/ - /-/g' "$$_f"; done; \
			for _f in $(FORMAT_ZSH_FILES); do $(SED_INPLACE) 's/ + /+/g' "$$_f"; done; \
...
## dockerfile format (currently skipped — dockfmt breaks BuildKit --mount syntax)
format/dockerfile:
...
format/nix:
	@if [ -n "$(FORMAT_NIX_FILES)" ]; then \
		...
		if command -v nixfmt >/dev/null 2>&1; then \
			nixfmt $(FORMAT_NIX_FILES); \
		else \
			echo "No Nix formatter found (nixfmt)"; \
		fi \
	fi

## format YAML files with prettier (falls back to yamlfmt)
format/yaml:
...
## format Rust source files with rustfmt
format/rust:
...
## format JSON files with prettier (falls back to jq)
format/json:
	@if find $(ROOTDIR) -type f -name "*.json" -not -path "*/.git/*" -print -quit | grep -q .; then \
		echo "Formatting JSON files..."; \
		if command -v prettier >/dev/null 2>&1; then \
			find $(ROOTDIR) -type f -name "*.json" -not -path "*/.git/*" | xargs -r prettier --write; \
		elif command -v jq >/dev/null 2>&1; then \
			find $(ROOTDIR) -type f -name "*.json" -not -path "*/.git/*" | while IFS= read -r f; do jq . "$$f" > "$$f.tmp" && mv "$$f.tmp" "$$f"; done; \
		else \
...
format/md:
	@if find $(ROOTDIR) -type f -name "*.md" -not -path "*/.git/*" -print -quit | grep -q .; then \
		echo "Formatting Markdown files..."; \
		if command -v prettier >/dev/null 2>&1; then \
			find $(ROOTDIR) -type f -name "*.md" -not -path "*/.git/*" | xargs -r prettier --write; \
		else \
...
## strip trailing whitespace from Makefiles (no standard auto-formatter)
format/makefile:
...
## rewrite bash shebangs to zsh and reformat (one-shot migration; skips BASH_KEEP_SCRIPTS)
...
## format shell-sourced conf files with shfmt; strip trailing whitespace from others
format/conf:
