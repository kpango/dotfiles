...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  float ratio = iResolution.y / iResolution.x, fw = max(fwidth(uv.x), fwidth(uv.y));
  ...
  vec3 fcol = mix(mix(mix(col1, col3, smoothstep(0.0, 0.3, noise)), col2, smoothstep(0.0, 0.5, noise)), col4, smoothstep(0.0, 1.0, noise));

  fragColor = vec4(ghosttyCol.rgb + mix(col4, fcol, smoothstep(fw, -fw, d) * smoothstep(fw, -fw, d2)), ghosttyCol.a);
}