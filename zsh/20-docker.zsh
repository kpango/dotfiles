...
devrun() {
	...
	local -a shared_vols=(
		...
		# ro: container must use host daemon socket for writes; direct SQLite access is read-only
		"-v ${XDG_DATA_HOME:-$HOME/.data}/atuin:$container_home/.data/atuin:ro"
		...
		"-v $rcpath/hunk.toml:$container_config/hunk/config.toml"
	...
	linux*)
		...
		if ! "${run_cmd[@]}"; then
			...
			echo "  containerd version: $(docker version --format '{{.Server.Components}}' 2>/dev/null | grep -o 'containerd [0-9.]*' || true)" >&2
		...
		docker exec -u 0 $container_name bash -c \
			"ln -sf $container_goroot/src/github.com/kpango/dotfiles/zshrc $container_home/.zshrc && \
			 ln -sf $container_goroot/src/github.com/kpango/dotfiles/zshenv $container_home/.zshenv" \
