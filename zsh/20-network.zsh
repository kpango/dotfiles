if (($+commands[nmcli])); then
	...
	_get_wifi_ifname() {
		nmcli d | rg wifi | head -1 | awk '{print $1}'
	}
	...
	nmcliwifi() {
		if [ $# -eq 2 ]; then
			...
			SECURITY=$(nmcli -f SSID,SECURITY dev wifi | rg "$SSID" | awk '{print $2}')
			KEY_MGMT="wpa-psk"
	...
	nmclr() {
		if [ $# -eq 1 ]; then
			...
			nmcli c show
			sudo nmcli c down "$1"
			# radio off/on toggle is the actual reset. Report failures but don't
			# abort on them: "c down" above has already dropped the connection,
			# so skipping the final "c up" on an off/on failure would leave the
			# user disconnected with no recovery attempt at all -- worse than
			# trying "c up" anyway against a possibly-stale radio state.
			sudo nmcli r wifi off || echo "ERROR: nmcli radio off failed" >&2
			sudo nmcli r wifi on || echo "ERROR: nmcli radio on failed — wifi radio may still be off" >&2
			sudo nmcli c up "$1"
		else
			echo "invalid argument, SSID and PSK is required"
		fi
...
if (($+commands[tailscale])); then
	...
	if (($+commands[ubnt-systool])); then
		...
		alias tailup="tailscaleup --ssh --reset --advertise-exit-node --advertise-routes=10.0.0.0/24,10.0.1.0/29 --stateful-filtering=false"
	else
		case ${OSTYPE} in
		darwin*)
			...
			;;
		linux*)
			alias tailup="tailscaleup --ssh --reset --accept-routes --accept-dns --advertise-routes=10.0.0.0/24 --stateful-filtering=false"
			;;
		esac
	fi
fi
