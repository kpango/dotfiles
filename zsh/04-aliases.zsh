...
if (($+functions[zsh-defer])); then
	zsh-defer -m -p -r -c "autoload -Uz select-word-style && select-word-style default"
else
...
if (($+commands[lsd])); then
	...
	erd_base='erd -H -d logical -I --level 1 --sort rsize -y inverted --dir-order first'
	erd_la='erd -H -d logical -I -l --group --octal --time-format iso --sort rsize -y inverted --dir-order first --threads 32 --hidden --color force'
...
zsync_state() {
	...
	if [[ -S /var/run/docker.sock && ! -w /var/run/docker.sock ]]; then
		sudo chown -R "${USER:-$(id -un)}:${GID:-$(id -gn)}" /var/run/docker.sock 2>/dev/null ||
			printf '     * warning: failed to chown docker.sock\n'
	fi
	local -a zombie_pids
...
mkcd() {
	if [[ -d "$1" ]]; then
		...
		printf "Confirm to Make Directory? %s [y/N]: " "$1"
		if read -q; then
			echo
			\mkdir -p "$1" && \cd "$1"
		fi
