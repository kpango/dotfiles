if (($+commands[git])); then
	...
	gfr() {
		...
		local remote_merged=$(git branch -r --merged $db | rg -v -e $db -e develop -e release | sed -E 's% *origin/%%')
		...
		if [ $# -eq 1 ] || [ $# -eq 2 ]; then
			...
			_grs_abort() {
				_git_restore_all "$branch_orig_head" "$branch" "$1 -- reverting to pre-command state" \
					tmp "" "$branch" "$branch_orig_head" "$target" "$target_orig_head"
			}
			...
			git checkout -b tmp || {
				_grs_abort "Failed to create tmp branch"
				return 1
			}
			...
			if [ $# -eq 2 ]; then
				git checkout "$2" . || {
					_grs_abort "Failed to checkout files from $2"
					return 1
				}
			fi
			...
			git commit --signoff -m "${message:-squash $branch}" || {
