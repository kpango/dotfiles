if (($+commands[git])); then
	...
	gfr() {
		...
		git branch -r --merged $db | rg -v -e $db -e develop -e release | sed -E 's% *origin/%%' | xargs -I% -P ${CPUCORES:-4} git push --delete origin % \
			2>/dev/null || true

		_git_fetch_reset $tb || return 1

		git branch --merged "$db" --format='%(refname:short)' |
			rg -v '^(master|develop|main)$|^release/' |
	...
	grsp() {
		if [ $# -eq 1 ] || [ $# -eq 2 ]; then
			local message="$(git log remotes/origin/$1..$(tb) --reverse --pretty=%s)"
			grs "$@"
			gcpf "$message"
		else
			echo "invalid argument, rebase branch name required"
	...
	git-squash-history() {
		...
		local msg=$*
		local tmp="orphan-$$"
		git checkout --orphan "$tmp" || return 1
		git add -A || {
			git checkout "$branch"
			return 1
		}
