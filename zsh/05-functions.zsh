zclean() {
	...
	# Unset stale $TMUX if the server is no longer running
	if (($+commands[tmux])) && [[ -n "$TMUX" ]] && ! tmux info &>/dev/null; then
...
zprecompile() {
	...
	# Also sweeps orphaned *.tmp.<pid> files: _zcache_eval writes each generation
	# to a PID-suffixed temp name (avoids a shared-name write race, see its own
	# comments) and cleans up after itself on the normal paths, but a shell
	# killed mid-write leaves that file behind with no other reaper -- *.zsh/
	# *.zwc above don't match ".zsh.tmp.<pid>" since it doesn't end in those
	# suffixes.
	setopt localoptions nullglob
	...
	(($+commands[zsh-patina])) &&
		_zcache_eval zsh-patina -1 \
			'zsh-patina activate; printf "export _ZSH_PATINA_PATH=\$(command -v zsh-patina)\n"' \
			"$commands[zsh-patina]"

	# zcompile rc files and autoloaded functions (_zcache_eval covers its own output).
	# _base_path/_apply_base_path run unconditionally on every shell start (both
	# interactive and non-interactive `zsh -c`, from zshrc, ahead of combined.zsh)
	# just like _zcache_eval/_atuin_init/_gen_env, so they belong in this list for
	# the same reason those already are. _source_deferred is called from zshrc
	# itself (ahead of combined.zsh, for combined.zsh/os-cache/gui-cache) and from
	# 3 zsh/*.zsh files bundled into combined.zsh -- same "runs on every startup"
	# reasoning. pinentry-tmux is NOT a zsh function despite
	# living under tmux.conf.d/pinentry-tmux/ -- it's the Go GPG pinentry binary `make
	# pinentry-tmux/build` installs to /usr/local/bin, so `zcompile` (which compiles
	# zsh script source to .zwc bytecode) was never applicable to it; the stale
	# "$_df/zfunc/pinentry-tmux" entry only ever no-op'd via the -f guard below.
	local f
	for f in \
		"$HOME/.zshrc" \
		"$HOME/.zshenv" \
		...
		"$_df/zfunc/_gen_env" \
		"$_df/zfunc/_base_path" \
		"$_df/zfunc/_apply_base_path" \
		"$_df/zfunc/_source_deferred"; do
...
chword() {
	if [ $# -ge 3 ] && [ $# -le 4 ]; then
		...
		# GNU sed's bare -i takes no backup suffix; BSD/macOS sed's -i requires
		# the (possibly empty) suffix as its own argument. GNU xargs' -r (skip
		# invocation on empty input) has no BSD/macOS flag, but BSD/macOS xargs
		# already behaves like -r by default, so it is only added on non-darwin.
		local -a sedi=(-i)
		local -a xargs_re=(-r)
		...
		if (($+commands[rg])); then
			rg --multiline -l $2 $1 | xargs -t "${xargs_re[@]}" -P ${CPUCORES:-4} \sed "${sedi[@]}" -E "$sed_cmd"
		elif (($+commands[ug])); then
			cd $1 && ug -l $2 | xargs -t "${xargs_re[@]}" -P ${CPUCORES:-4} \sed "${sedi[@]}" -E "$sed_cmd" && cd -
		elif (($+commands[jvgrep])); then
			jvgrep -I -R $2 $1 --exclude $jvgrule -l -r | xargs -t "${xargs_re[@]}" -P ${CPUCORES:-4} \sed "${sedi[@]}" -E "$sed_cmd"
		else
			_find_text_files $1 | xargs -0 "${xargs_re[@]}" -P ${CPUCORES:-4} grep -rnwe $2 | xargs -t "${xargs_re[@]}" -P ${CPUCORES:-4} \sed "${sedi[@]}" -E "$sed_cmd"
		fi
	else
		echo "Not enough arguments"
	fi
...
duk() {
	local dir="${1:-/}"
	local k="${2:-10}"
	sudo du -h -d 1 "$dir" 2>/dev/null \
		| sort -rh \
		| awk -v d="${dir%/}" -v k="$k" '
			$2 == d "/proc" || $2 == d "/sys" || $2 == d "/dev" || $2 == d "/run" { next }
			$2 != d && $2 != d "/" { print; if (++cnt >= k) exit }
		'
}
...
## Truncate $_f to its last $_n lines in place. Pass "sudo" as $3 for files not
## owned by the current user (e.g. /var/log/*) -- other callers (e.g. the
## user's own ~/.claude logs) don't need the extra privilege escalation/prompt.
_sysclean_log_tail() {
	local _f="$1" _n="${2:-1000}" _use_sudo="${3:-}" _tmp
	[[ -f "$_f" ]] || return 0
	_tmp=$(mktemp)
	if [[ "$_use_sudo" == "sudo" ]]; then
		tail -n "$_n" "$_f" > "$_tmp" 2>/dev/null && sudo mv "$_tmp" "$_f" || { rm -f "$_tmp"; return 1; }
	else
		tail -n "$_n" "$_f" > "$_tmp" 2>/dev/null && mv "$_tmp" "$_f" || { rm -f "$_tmp"; return 1; }
	fi
}
...
_sysclean_tmux_stale() {
	(($+commands[tmux])) || return 0
	setopt localoptions nullglob
	local _uid="${UID:-$(id -u)}"
	local -a candidates=()
	# Default socket dir created by tmux without -S (e.g. /tmp/tmux-1000/default)
	[[ -d "/tmp/tmux-${_uid}" ]] && candidates+=("/tmp/tmux-${_uid}"/*)
	# Custom sockets directly under /tmp (e.g. /tmp/tmux.sock, /tmp/tmux-main.sock)
	candidates+=(/tmp/tmux*.sock)
	# XDG_RUNTIME_DIR / /run/user/<uid>
	local _run="${XDG_RUNTIME_DIR:-/run/user/${_uid}}"
	[[ -d "$_run" ]] && candidates+=("$_run"/tmux*.sock)
	# $HOME/.tmux/
	[[ -d "$HOME/.tmux" ]] && candidates+=("$HOME/.tmux"/*.sock)
	local _sock
	for _sock in "${candidates[@]}"; do
		[[ -S "$_sock" ]] || continue
		! tmux -S "$_sock" info &>/dev/null && rm_targets+=("$_sock")
	done
	return 0
}
...
_sysclean_claude() {
	...
	for _f in "$_d/bash-commands.log" "$_d/cost-tracker.log" "$_d/daemon.log"; do
		_sysclean_log_tail "$_f" 1000
	done
	return 0
}

_sysclean_src_build_cache() {
	local _src="${GOPATH:-$HOME/go}/src/github.com"
	[[ -d "$_src" ]] || return 0
	local _org _repo _orgname
	for _org in "$_src"/*(N/); do
		_orgname="${_org:t}"
		for _repo in "$_org"/*(N/); do
			[[ -d "$_repo/node_modules" ]] && rm_targets+=("$_repo/node_modules")
			[[ "$_orgname" == "kpango" || "$_orgname" == "vdaas" ]] && continue
			[[ -d "$_repo/target" ]] && rm_targets+=("$_repo/target")
		done
	done
	return 0
}
...
_sysclean_rustup() {
	...
	_sysclean_step "rustup: update stable" \
		rustup update stable
	local -a old_chains
	old_chains=(${(f)"$(rustup toolchain list 2>/dev/null | awk '$1 !~ /^stable/ {print $1}')"})
	local _tc
	for _tc in "${old_chains[@]}"; do
		[[ -n "$_tc" ]] && _sysclean_step "rustup: uninstall $_tc" \
			rustup toolchain uninstall "$_tc"
	done
}
...
sysclean() {
	...
	if ! pgrep -f "google-chrome" > /dev/null 2>&1 && ! pgrep -x chromium > /dev/null 2>&1; then
		...
		for _chrome_dir in 			"$HOME/.cache/google-chrome-beta" 			"$HOME/.cache/google-chrome" 			"$HOME/.cache/chromium"; do
			[[ -d "$_chrome_dir" ]] && rm_targets+=("$_chrome_dir")
		done
	fi
	...
	# --- Orphaned app data (tool no longer installed) ---
	if ! (($+commands[nvim])); then
		local _xdg_data="${XDG_DATA_HOME:-$HOME/.local/share}"
		local _alt_data="$HOME/.data"
		[[ -d "$_xdg_data/nvim" ]] && rm_targets+=("$_xdg_data/nvim")
		[[ -d "$_alt_data/nvim" ]] && rm_targets+=("$_alt_data/nvim")
		[[ -d "$_xdg_data/TabNine" ]] && rm_targets+=("$_xdg_data/TabNine")
		[[ -d "$_alt_data/TabNine" ]] && rm_targets+=("$_alt_data/TabNine")
	fi
	...
	# --- go/src: node_modules and external Rust target/ ---
	_sysclean_src_build_cache
	...
	if (( ${#rm_targets[@]} > 0 )); then
		_sysclean_step "parallel file deletion (${#rm_targets[@]} items)" \
			sh -c 'cpu=$1; shift; printf "%s\0" "$@" | sudo xargs -0 -P "$cpu" rm -rf' \
			_ "${CPUCORES:-4}" "${rm_targets[@]}"
	fi

	# Reset stale $TMUX if the server is no longer running (socket may have just been deleted above)
	if (($+commands[tmux])) && [[ -n "$TMUX" ]] && ! tmux info &>/dev/null; then
		unset TMUX TMUX_PANE
		printf '\033[34m[sysclean]\033[0m tmux: stale $TMUX cleared\n'
		typeset -g _SYSCLEAN_OK=$((_SYSCLEAN_OK+1))
	fi
	...
	if (($+commands[docker])); then
		# GNU xargs' -r (skip invocation on empty input) has no BSD/macOS flag;
		# BSD/macOS xargs already behaves like -r by default. Docker Desktop on
		# macOS also has no /var/lib/docker on the host, so find yields nothing
		# there either way.
		local _xargs_re="-r"
		[[ "$OSTYPE" == darwin* ]] && _xargs_re=""
		_sysclean_step "docker: truncate container logs" 			sh -c "sudo find /var/lib/docker/containers -name '*-json.log' -print0 2>/dev/null | sudo xargs -0 $_xargs_re truncate -s 0"
		_sysclean_step "docker: system prune (stopped containers / dangling images / unused volumes)" 			docker system prune -f --volumes
		_sysclean_step "docker: all named volumes (unused)" 			docker volume prune -a -f
	...
	_sysclean_step "/var/log/pacman.log: keep last 1000 lines" 		_sysclean_log_tail /var/log/pacman.log 1000 sudo
	...
	if (($+commands[go])); then
		...
		_sysclean_step "go: module cache" 			go clean -modcache
	fi

	# --- rustup: update stable, remove old toolchains ---
	_sysclean_rustup
