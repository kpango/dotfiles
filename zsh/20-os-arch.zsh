if (($+commands[pacman])); then
	...
	kacclean() {
		...
		# Owner-only: gcloud's config dir holds OAuth credentials, so there is
		# no reason for group/other to read or write it. Both run via sudo (not
		# just chown) so this also applies cleanly regardless of the
		# directory's current ownership.
		# -d, not just -n "$XDG_CONFIG_HOME": gcloud may simply not be installed,
		# in which case $XDG_CONFIG_HOME/gcloud doesn't exist and chown below
		# would fail with ENOENT -- indistinguishable, without this guard, from
		# a real ownership-change failure and reported as one.
		# A failed chmod here (permission denied, read-only fs, ...) means the
		# owner-only tightening below silently never happened -- surfacing it
		# rather than swallowing it, same reasoning as the chown below.
		[[ -n "${XDG_CONFIG_HOME:-}" && -d "$XDG_CONFIG_HOME/gcloud" ]] && { sudo chmod -R u+rwX,go-rwx "$XDG_CONFIG_HOME/gcloud" 2>/dev/null ||
			print -u2 "kacclean: failed to chmod $XDG_CONFIG_HOME/gcloud to owner-only — it may still be more permissive than intended"; }
		# Unlike the old world-writable 777, a failed chown here can leave the
		# dir owned by someone other than $USER with group/other now locked
		# out by the chmod above -- silently swallowing that (the old `|| true`
		# was harmless under 777, since anyone could still get in regardless
		# of ownership) would turn "cleanup failed" into "gcloud config is now
		# inaccessible with no explanation why".
		[[ -n "${XDG_CONFIG_HOME:-}" && -d "$XDG_CONFIG_HOME/gcloud" ]] && { sudo chown -R "$USER" "$XDG_CONFIG_HOME/gcloud" 2>/dev/null ||
			print -u2 "kacclean: failed to chown $XDG_CONFIG_HOME/gcloud back to $USER — it may now be inaccessible"; }
	...
	test_pacman_mirror() {
		...
		for _attempt in 1 2 3; do
			if rg -q "Temporary failure in name resolution|could not resolve host|Name or service not known" "$log_file"; then
	...
	arch_fix_db_conflicts() {
		...
		while IFS= read -r line; do
			...
			# Native zsh regex match instead of 3x `sed` (one fork per captured
			# group, all parsing the identical line) -- `$match[N]` already holds
			# every capture group after a single match. The `-z "$match[3]"` half
			# of the skip condition preserves the old code's `-z "$filepath"`
			# guard (an empty captured path is still skipped, not just a total
			# non-match) -- verified against sample conflict lines (see also a
			# fabricated empty-filepath line) that this and the old sed-based
			# extraction agree on every case tested.
			if [[ "$line" =~ "file owned by '([^']*)' and '([^']*)': '([^']*)'" ]] && [[ -n "$match[3]" ]]; then
				pkg1=$match[1] pkg2=$match[2] filepath=$match[3]
			else
				continue
			fi
			...
			local loser_entry
			loser_entry=$(ls "$db_dir" 2>/dev/null | rg "^${loser}-" | head -1)
			...
			((fixed++))
		done < <(sudo pacman -Dk 2>&1 | rg "file owned by")
	...
	_fix_makepkg_conf_if_broken() {
		...
		if rg -q '^[[:space:]]*PKGEXT=' "$conf" 2>/dev/null &&
			rg -q '^[[:space:]]*SRCEXT=' "$conf" 2>/dev/null &&
			rg -q '^[[:space:]]*BUILDENV=' "$conf" 2>/dev/null; then
			return 0
		fi
		...
		if [[ ! -f "$_custom_conf" ]] || ! rg -q 'PKGDEST' "$_custom_conf"; then
	...
	# Multi-stage DKMS auto-repair for a single module.
	# Stage 1: direct install
	# Stage 2: nvidia BTF patch+retry (when BTF error detected in make.log)
	# Stage 3: clean reinstall (remove stale build artifacts)
	# Stage 4: AUR package rebuild (for foreign/AUR-sourced DKMS modules)
	_dkms_rebuild_one() {
		local _mod="$1" _ver="$2" _kern="$3"
		local _spec="${_mod}/${_ver}"
		local _log="/var/lib/dkms/${_mod}/${_ver}/build/make.log"

		# Stage 1: direct install
		if _dkms_install_safe "${_spec}" -k "${_kern}"; then
			echo "    OK"
			return 0
		fi

		# Diagnose failure from make.log
		local _diag=""
		if [[ -f "$_log" ]]; then
			rg -q "pahole-flags\.sh|gen-btf\.sh" "$_log" 2>/dev/null \
				&& _diag="btf"
			rg -qi "cannot find.*headers|kernel headers.*not found" "$_log" 2>/dev/null \
				&& _diag="${_diag:+${_diag},}headers"
		fi

		# Stage 2: nvidia BTF patch (nvidia module only)
		if [[ "$_diag" == *btf* && "$_mod" == "nvidia" ]]; then
			echo "    ==> BTF generation error — retrying after nvidia_dkms_fix"
			nvidia_dkms_fix
			if _dkms_install_safe "${_spec}" -k "${_kern}"; then
				echo "    OK (after BTF patch)"
				return 0
			fi
		fi

		# Stage 3: clean reinstall — purge stale build artifacts and reinstall
		echo "    ==> Attempting clean reinstall for ${_spec}"
		sudo dkms remove "${_spec}" -k "${_kern}" --no-depmod &>/dev/null || true
		if _dkms_install_safe "${_spec}" -k "${_kern}"; then
			echo "    OK (clean reinstall)"
			return 0
		fi

		# Stage 4: AUR rebuild — locate the foreign package that ships this DKMS module
		local _aur_pkg
		_aur_pkg=$(pacman -Qm 2>/dev/null \
			| awk -v m="${_mod}" 'index(tolower($1), m) && index($1, "dkms") {print $1}' \
			| head -1)
		if [[ -n "$_aur_pkg" ]]; then
			echo "    ==> Rebuilding AUR package: ${_aur_pkg}"
			if paru -S --noconfirm --rebuild "$_aur_pkg" 2>/dev/null; then
				# paru may have already run dkms install as part of the PKGBUILD
				dkms status "${_spec}" 2>/dev/null | rg -q ": installed$" \
					&& { echo "    OK (AUR rebuild ran dkms install)"; return 0; }
				if _dkms_install_safe "${_spec}" -k "${_kern}"; then
					echo "    OK (after AUR rebuild)"
					return 0
				fi
			fi
		fi

		# All stages exhausted — show log for human triage
		echo "    FAILED (all recovery attempts exhausted)"
		if [[ -f "$_log" ]]; then
	...
	arch_maintenance() {
		run_command "pacman db upgrade" sudo pacman-db-upgrade || true
		...
		for _preset in /etc/mkinitcpio.d/*.preset; do
			...
			run_command "regenerate initramfs ($_preset_name)" sudo mkinitcpio -p "$_preset_name" || true
		done
		...
		if (($+commands[dkms])); then
			...
			_dkms_broken=$(dkms status 2>/dev/null | rg -v ": installed$")
			if [[ -n "$_dkms_broken" ]]; then
				...
				while IFS= read -r _line; do
					...
					# Auto-install missing kernel headers before attempting rebuild.
					if [[ ! -d "/usr/lib/modules/${_kern}/build" ]]; then
						local _flavor="${_kern##*-}" _hdr_pkg=""
						case "$_flavor" in
							zen)      _hdr_pkg="linux-zen-headers" ;;
							lts)      _hdr_pkg="linux-lts-headers" ;;
							hardened) _hdr_pkg="linux-hardened-headers" ;;
							rt)       _hdr_pkg="linux-rt-headers" ;;
							arch)     _hdr_pkg="linux-headers" ;;
						esac
						if [[ -n "$_hdr_pkg" ]] && sudo pacman -S --noconfirm --needed "$_hdr_pkg" 2>/dev/null; then
							echo "  ==> Installed missing headers: ${_hdr_pkg}"
						else
					...
					_dkms_rebuild_one "${_mod}" "${_ver}" "${_kern}"
				done <<<"$_dkms_broken"
			else
	...
	archup() {
		local _log_dir="${XDG_STATE_HOME:-$HOME/.local/state}/archup"
		mkdir -p "$_log_dir" 2>/dev/null || true
		local _log="$_log_dir/archup-$(date +%Y%m%d-%H%M%S).log"

		{
			...
			arch_update_aur_ignored
			arch_maintenance || echo "==> WARNING: arch_maintenance had failures; see $_log"
			arch_fix_db_conflicts
			arch_fix_missing_files
			_warn_pending_pacnew
			(($+functions[fup])) && { fup || echo "==> WARNING: firmware update (fup) failed"; }
		} 2>&1 | tee "$_log"

		# Rotate: keep the 20 most recent logs
		ls -1t "$_log_dir"/archup-*.log 2>/dev/null | tail -n +21 | xargs -r rm -f 2>/dev/null || true
	}
	alias up=archup
