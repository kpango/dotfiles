if (($+commands[pacman])); then
		...
		if (($+commands[$manager])); then
			...
			if [[ "$manager" == "pakku" ]]; then
				...
				if [[ ! -f "$_pakku_dir/makepkg.conf" ]]; then
					mkdir -p "$_pakku_dir"
					cp /etc/makepkg.conf "$_pakku_dir/makepkg.conf"
				fi
			fi
	...
	arch_update_aur_ignored() {
		...
		[[ ${#_pkgs[@]} -eq 0 ]] && return 0
		echo "==> Updating AUR packages skipped during main upgrade: ${_pkgs[*]}"
		kacman -S --noconfirm "${_pkgs[@]}" ||
			echo "WARNING: could not update some AUR packages (${_pkgs[*]})"
	}
	...
	_fix_makepkg_conf_if_broken() {
		...
		# pakku override required by try_package_manager (uses PKGDEST in
		# /tmp/pakku-${USER}). Keep this in sync with the seed location.
		printf '\n# pakku override (preserved by _fix_makepkg_conf_if_broken)\nPKGDEST=/tmp/pakku-%s\n' "$USER" |
	...
	_merge_safe_makepkg_conf_d_pacnew() {
		...
		for pacnew_file in /etc/makepkg.conf.d/*.pacnew(N); do
