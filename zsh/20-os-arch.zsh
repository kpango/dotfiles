if (($+commands[pacman])); then
	...
	arch_update_aur_ignored() {
		...
		kacman -S --noconfirm "${_need_update[@]}" ||
			echo "WARNING: could not update some AUR packages (${_need_update[*]})"
	}
		...
		if (($+commands[dkms])); then
			...
			if [[ -n "$_dkms_broken" ]]; then
				...
				while IFS= read -r _line; do
					...
					[[ -z "$_kern" ]] && _kern=$(uname -r)
					echo "==> Rebuilding ${_mod}/${_ver} for ${_kern}"
					_dkms_install_safe "${_mod}/${_ver}" -k "${_kern}" &&
						echo "    OK" || echo "    FAILED — check /var/lib/dkms/${_mod}/${_ver}/build/make.log"
				done <<<"$_dkms_broken"
			else
