...
if [[ -z "$_ZSH_PATH_LOADED" ]]; then
	...
	export PATH="$HOME/.bun-shims:/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin:/bin:/sbin:/usr/local/bun/bin:/usr/local/go/bin:/opt/local/bin:${GOBIN:-$HOME/go/bin}:$HOME/.local/bin:${CARGO_HOME:-$HOME/.cargo}/bin:$GCLOUD_PATH/bin:/usr/lib/docker/cli-plugins/:$PATH"
