...
valdmanifest() {
	make files helm/schema/all helm/schema/crd/all k8s/manifest/update k8s/manifest/helm-operator/update k8s/manifest/benchmark-operator/update helm/docs/vald helm/docs/vald-helm-operator helm/docs/vald-benchmark-operator clean/yaml files
}
valdup() {
	...
	_valdup_step() {
		local label=$1
		shift
		local snap untracked_before untracked_after f
	...
	_valdup_step format make format
	_valdup_step chword chword $GOPATH/src/github.com/vdaas/vald "interface\{\}" "any"
	_valdup_step proto make proto/replace format/go format/go/test workflow/fix
