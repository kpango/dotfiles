...

{
  ...
  # Kernel parameters — Threadripper 3990X / TRX40 / RTX 2070 SUPER
  boot.kernelParams = [
    "amd_iommu=on"
    "iommu=pt"
    "amd_pstate=disable" # TRX40 CPPC reports 0 for min/max/nominal; active mode fails
    "idle=nomwait" # Use HALT instead of MWAIT for C-state entry on AMD
    "processor.max_cstate=1" # Limit C-states for low-latency workloads
    "pcie_aspm=off" # Disable ASPM; avoids latency jitter on NVMe RAID + X710
    "cgroup_no_v1=all"
    "nvidia_drm.fbdev=1"
    ...
    "zswap.zpool=z3fold" # z3fold: better compression density than zsmalloc for large RAM
    "nowatchdog"
    "quiet"
    "loglevel=1"
    "acpi_backlight=none"
