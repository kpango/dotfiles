...
in
{
  ...
  network = {
    # System nameservers (used by networking.nameservers; resolved reads these as DNS=)
    nameservers = [
    ...
    # Docker daemon DNS — 100.100.100.100 first for Tailscale MagicDNS via host NAT
    dockerDns = [
      "100.100.100.100"
      cloudflareDnsV4Primary
      cloudflareDnsV4Secondary
    ...
    # Dnsmasq caching servers (kept for reference; no longer used by NM or Docker)
  ...
  darwin = {
    ...
    masApps = { };
    # `aliases.colima` (colima's own VM sizing flags) lived here; removed along with colima
    # itself — see modules/darwin/containerization.nix for its apple/container replacement.
    dock = {
