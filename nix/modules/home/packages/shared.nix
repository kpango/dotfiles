...
# prebuilt in the binary cache, so no crates.io access is needed at all.
{
  home.packages = with pkgs; [
    ...
    # (successor to Gemini CLI); ~/.agy/ and ~/.gemini/ config is deployed both
    # by Makefile.d/install.mk's DOTFILES_MAP/agy/install and, on a home-manager
    # host, by dotfiles/agent-tools.nix (and dotfiles/shared.nix, for the two
    # entries that predate that module) — see those for the full mapping.
    antigravity-cli
    github-copilot-cli
