...
if [ -z $ZSH_LOADED ]; then
    ...
    zshstorehistory() { # store function, this function stores zsh history to pass and remote github repository
        emulate -L zsh
	...
	# store history to pass with key zsh/history
        pass insert -m "$PASS_KEY" < "$HISTFILE"
	...
	# if diff-index exitsts, push to remote
