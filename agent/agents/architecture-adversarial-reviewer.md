...
### 8. Ponytail Anti-Overengineering & YAGNI Audit

- **Speculative Abstractions**: Flag new interfaces, factories, generic wrappers, or plugin hooks that have only 1 concrete implementation or no current caller.
- **Codebase Reuse & Stdlib Priority**: Flag any new custom implementation or external dependency where existing codebase utilities or language standard libraries already provide the needed capability.
- **Gratuitous Architectural Churn**: Reject changes that perform unsolicited architectural refactorings or wide-scale file restructuring outside the stated task scope.
