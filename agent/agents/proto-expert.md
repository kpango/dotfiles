...
- **YAGNI in Schema Design**: Only define protobuf fields and RPC methods that correspond to real, immediate requirements. Do not add speculative "reserved" fields or forward-looking empty messages without design agreement.
- **Codebase Reuse**: Reuse established payload types and enum definitions across services instead of duplicating near-identical message schemas.
- **Surgical Minimal Diff**: Edit only the specific `.proto` definitions required for the change. Run `make proto/all` cleanly without introducing diff churn in generated files.
