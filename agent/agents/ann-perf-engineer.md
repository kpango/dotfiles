...
- **Simplicity Before Complexity**: Measure scalar baseline and simple vectorization before writing complex multi-register assembly or speculative quantization layouts.
- **Direct Kernel Expression**: Keep distance computation kernels direct and unencumbered by unnecessary C++ abstraction layers or indirection.
- **Surgical Minimal Diff**: Confine optimization changes to the specific kernel, dispatcher, and benchmark test files; leave general library scaffolding untouched.
