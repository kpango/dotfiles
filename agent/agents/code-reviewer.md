...
- Twin/paired artifacts checked together: if the diff touches one side of a structurally-mirrored pair (sibling
  functions, OS/platform-specific variants of the same config or service, parallel implementations), verify the
  counterpart was updated to match, or that the divergence is called out as intentional
- N-location duplicated commitments (no single source of truth): if the diff introduces or renames an entity that
  is independently asserted in 3+ places (e.g. a partner-list table, a summary paragraph, and a peer document all
  restating the same fact), grep for every other location making the same assertion and confirm all were updated
  in this diff — do not rely on catching the last straggler in a later review round
...
### Ponytail Anti-Overengineering & Safe Minimal Code

- **YAGNI Audit**: Flag any speculative generic parameters, unused abstraction layers, premature interfaces with single implementations, or uncalled configuration options.
- **Codebase Reuse**: Check whether author reinvented functions already present in the codebase.
- **Stdlib First**: Reject external dependencies introduced when standard library utilities suffice.
- **Surgical Minimal Diff**: Reject gratuitous renames, unsolicited refactorings, or cosmetic formatting changes in unrelated files.
- **Safety Guard**: Ensure brevity never compromises safety. Reject diffs that skip bounds checks, suppress errors (`_ = err`, empty catch/except), or bypass security invariants (Vald Laws, shell injection prevention).
