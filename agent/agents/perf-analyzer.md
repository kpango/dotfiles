...
> vald配下では `go build`（`-gcflags`付きも含む）は vald-law2-gate.sh がブロックする(Vald Law 2)。
> vald 用の escape 解析 make target は存在しない(perf-simd-adversarial-reviewer.md で確認済み)ため、
> vald では実行せず「検証不能」と報告する。dotfiles 等 vald 以外では制約なく実行してよい。
...
> vald配下では `cargo build` は vald-law2-gate.sh がブロックする(Vald Law 2)。既存の make ターゲット
> 経由でビルドし、フレームグラフ/LLVM MCA 用バイナリはそのビルド成果物を使う。
...
After working in a project, update your memory directory's `MEMORY.md` with: hot paths and their measured
cost, optimizations tried and their actual (not theoretical) impact, profiling commands/flags that worked
for this project's build setup, and any anti-pattern found here not already listed above. Skip the update
if nothing new came up.

## Ponytail Anti-Overengineering Directives

- **Measure First, Simplify Always**: Optimize only proven bottlenecks identified by profiling. Reject speculative micro-optimizations that increase complexity without measurable gain.
- **Idiomatic Minimal Solutions**: Prefer reducing algorithmic complexity or eliminating redundant allocations over complex custom memory managers or thread pools.
- **Surgical Minimal Diff**: Apply performance improvements with surgical precision to the hotspot function; do not refactor callers or adjacent subsystems.
