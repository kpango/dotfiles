...
```bash
go build ./...   # vald配下では vald-law2-gate.sh が `^go (build|run|install)` をブロックする
                  # (Vald Law 2)。vald の場合は代わりに既存の make ターゲットを使う。
go vet ./...
go test ./...
govulncheck ./...
```
...
Strictly adhere to the Ponytail 7-step anti-overengineering logic ladder and safe minimal code patterns:

- **YAGNI & Concrete Types**: Do not define premature interfaces for single implementations. Accept interfaces only where multiple consumers exist; return concrete structs. Reject speculative generic type parameters.
- **Stdlib First**: Use Go standard library packages (`slices`, `maps`, `cmp`, `net/http`, `crypto`, `encoding/json`) instead of external dependencies.
- **Codebase Reuse**: Grep existing packages for utilities and types before writing new ones.
- **Surgical Minimal Diff**: Keep diffs minimal. No gratuitous renames, formatting changes, or unrelated refactorings.
- **Safety Invariants**: Never omit bounds checks, nil pointer checks, or error handling for brevity. Never ignore errors with `_ = err` (Vald Law 4).
