...
- **Targeted CI Fixes**: Fix the specific environment, image, or prerequisite issue without rewriting entire workflow files or Makefiles.
- **Platform Native Primitives**: Use standard shell and Make built-ins rather than adding complex scripting dependencies into CI runners.
- **Surgical Minimal Diff**: Confine CI pipeline fixes strictly to the failing step or configuration parameter.
