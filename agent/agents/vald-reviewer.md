...
- **Laws 3-5**: `vald-law345-check.sh` asks for confirmation on obvious violations at write time (PreToolUse, upgraded from a PostToolUse-only warning — see `agent/README.md`'s "hooks/ のルールデータ共有化" for the claude/agy/pi consolidation that unified this with agy's stronger enforcement). Scans only the content a Write/Edit/MultiEdit call is about to introduce, not the whole file, and skips `_test.go`
...
まず対象を絞る。Go/Rust 双方が対象のため `rust/` もパススペックに含める（前段の Go 限定パススペックの
まま `rust/` を落とすと、Rust のみの変更で `$changed` が空になり Law 3 の Rust `panic!(` チェックまで
丸ごとスキップされてしまう）:

```bash
changed=$(git diff --name-only $(git merge-base HEAD main)..HEAD -- pkg/ internal/ cmd/ rust/)
```

`$changed` が空（設定ファイルのみの変更等）なら Law 3-5 スキャンは省略してよい（Go/Rust コード変更を
伴わない限り新規に発生しない）。非空なら "indirect violations" を拾うため、変更ファイル本体だけでなく
同一ディレクトリの兄弟ファイルまで対象に加える（`architecture-adversarial-reviewer.md` が既に採用して
いる範囲の絞り方と同型）:

```bash
changed_and_siblings=$(printf '%s\n' "$changed" | xargs -n1 dirname | sort -u | \
  xargs -I{} find {} -maxdepth 1 -type f \( -name '*.go' -o -name '*.rs' \) | sort -u)

# Law 3: panic/log.Fatal (Go + Rust)
grep -n 'panic(\|log\.Fatal\|log\.Fatalf\|log\.Fatalln' $changed_and_siblings
grep -n 'panic!(' $changed_and_siblings 2>/dev/null || true

# Law 4: discarded errors
grep -n '_ =' $changed_and_siblings | grep -v '_test\.go' | grep -i 'err'

# Law 5: standard library usage
grep -n '"log"\|"errors"\|"sync"\|"strings"' $changed_and_siblings | grep -v 'internal/log\|internal/errors\|internal/sync\|internal/strings'
```

無関係な既存コードの標準監視は本 agent の役割（"after any Vald code change"）の対象外であり、
`vald-law345-check.sh`（write-time の個別ファイルチェック）と合わせて二重の安全網は保たれる。
...
- **YAGNI & Vald Laws**: Enforce that new components and options are strictly necessary. Speculative configuration parameters in `values.yaml` or `config.go` must be rejected.
- **Codebase Reuse**: Vald's `internal/**` packages provide rich concurrency, error, and logging primitives — insist on reusing existing internal helpers instead of reinventing them.
- **Surgical Minimal Diff**: Reject diffs that perform wide-scale refactoring outside the targeted component or break the synchronized 3-location config pairing.
- **Safety & Law Compliance**: Minimalism must never compromise Vald Laws (Laws 1-5). Error handling must remain complete; panic and bare log.Fatal remain strictly forbidden.
