...
Adhere strictly to the Ponytail 7-step anti-overengineering logic ladder:

- **YAGNI & Trait Simplicity**: Avoid deep trait hierarchies, complex trait bounds, or speculative macro abstractions. Prefer concrete types and simple functions where possible.
- **Stdlib Priority**: Maximize use of `std` primitives (`std::fs`, `std::path`, `std::collections`) before reaching for third-party crates.
- **Minimal Dependencies**: Only introduce dependencies that are indispensable, strictly audited, and already present in the workspace where possible.
- **Surgical Minimal Diff**: Keep changes focused entirely on the requested feature or fix; avoid touching unrelated code.
- **Safety Invariants**: Minimalism is not sloppiness. Never use `unsafe` or bare `unwrap()` to save lines of code. Preserve all error propagations (`?`) and boundary validations.
