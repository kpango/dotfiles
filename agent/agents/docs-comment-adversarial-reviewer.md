...
本agentが担う領域(技術文書のOverclaim検出・ドキュメント一貫性・相互参照妥当性)に専用のagentは
他のいずれにも無い(具体的な既存agent数はagentが増減するたびに陳腐化するため本文中には固定しない
— 実数は`ls agent/agents/*.md`で確認すること)。ただし完全な新規領域ではない点に注意する: 個別コメントのWHY/WHAT判定は
...
### e. Ponytail 規範遵守（不要な記述・過剰ドキュメント・投機的言及の排除）

- 未実装の将来機能や投機的拡張フックに関するドキュメント記載を YAGNI 違反として検出する。
- ソースコードと同等の自明な説明による不要な行数肥大化を指摘し、簡潔で高密度な技術記述を求める。
- 差分内のドキュメント修正が必要最小限（Surgical Minimal Diff）に収まっているかを確認する。
