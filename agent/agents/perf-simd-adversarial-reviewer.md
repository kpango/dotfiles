...
## Axis C: Ponytail Anti-Overengineering in Performance Work

- **Speculative micro-optimization vs Simplicity**: Flag complex, unmeasured micro-optimizations (e.g. convoluted assembly or multi-level indirection) that lack benchmark justification.
- **Minimal expression in hot paths**: Verify that performance-critical loops are expressed cleanly without unnecessary abstraction layers or object allocations.
- **Surgical diff in perf changes**: Ensure performance tuning touches only the bottlenecked hot path, avoiding wide-scale unsolicited refactoring.
