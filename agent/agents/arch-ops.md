...
- **Platform Native First**: Use native Arch Linux, systemd, and POSIX primitives (`systemctl`, `journalctl`, `udev`, coreutils) before writing complex custom orchestration scripts.
- **Minimal Dependency**: Prefer standard pacman repository packages over complex AUR toolchains; avoid installing heavy daemons when lightweight built-in systemd units suffice.
- **Surgical Minimal Diff**: Dotfile modifications must be minimal, targeted, and directly related to the user request. Avoid reformatting entire config files.
