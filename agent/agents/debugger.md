...
- **Surgical Root-Cause Fixes**: The ideal fix is the minimal change that directly resolves the root cause. Avoid "while I'm here" refactoring or speculative architecture shifts during bug fixing.
- **No New Dependencies for Fixes**: Solve bugs using existing codebase utilities and language standard libraries; do not import external packages to fix a defect.
- **Preserve Safety & Invariants**: Ensure fixes add rigorous edge-case coverage and boundary checks rather than working around problems via error suppression.
