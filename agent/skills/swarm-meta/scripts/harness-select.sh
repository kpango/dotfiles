...
. "$(dirname "${BASH_SOURCE[0]}")/../../swarm-implement/scripts/swarm-common-lib.sh"
...
repo_root="${2:-}"
if [ -z "$repo_root" ]; then
  repo_root="$(swarm_repo_root 2>/dev/null || pwd)"
fi
...
# Additive domain specialist recommendations based on goal keywords
DOMAIN_AGENTS = [
    (["golang", "go言語", "go-expert", "go.mod", "goroutine", "goの", "goで", "go製", "goコード", "go test"], "go-expert"),
    (["rust", "cargo", "crates.io", "rust-expert", "rustの", "rustで"], "rust-expert"),
    (["cpp", "c++", "cmake", "cpp-expert", "vcpkg", "c++の"], "cpp-expert"),
    (["python", "pytorch", "pytest", "python-expert", "pip", "pyproject", "pythonの"], "python-expert"),
    (["zig", "zig-expert", "build.zig", "zigの"], "zig-expert"),
    (["k8s", "kubernetes", "helm", "kustomize", "k8s-expert"], "k8s-expert"),
    (["nix", "nixos", "nix-darwin", "home-manager", "nix-expert"], "nix-expert"),
    (["github action", "github actions", "ci/cd", "ci-investigator", "github-actions-expert"], "github-actions-expert"),
    (["protobuf", "proto", "grpc", "proto-expert", "buf lint"], "proto-expert"),
    (["ann ", "vector search", "simd", "arcflare", "ngtaq", "ann-perf-engineer"], "ann-perf-engineer"),
    (["perf", "benchmark", "profiling", "pprof", "criterion", "perf-analyzer"], "perf-analyzer"),
    (["vald", "vdaas/vald", "vald-reviewer"], "vald-reviewer"),
    (["arch linux", "arch linux env", "arch linuxの", "pacman", "systemd", "sway", "yay", "paru", "arch-ops", "zen kernel", "wayland"], "arch-ops"),
    (["debug", "root cause", "crash", "panic", "test failure", "debugger"], "debugger"),
    (["architect", "architecture", "adr", "設計"], "swarm-architect"),
]

for keywords, agent_name in DOMAIN_AGENTS:
    if any(kw in goal_lower for kw in keywords):
        if agent_name not in lenses:
            lenses.append(agent_name)
