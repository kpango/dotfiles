...
> domain `swarm-graph` with the schema in the `## SKILL.state Schema` section, then
...
> `skill_state_*` tools. Σ never overrides `graph-status.sh` (the sole authority for
> frontier/critical-path/stale state).
...
`graph-status.sh` remains the **sole authority** for frontier/critical-path/stale
state (this skill's existing invariant); this Σ is only a subordinate projection for
the Pi runtime and never becomes the readiness authority. The union list-merge
keeps the projected sets monotone across steps.
