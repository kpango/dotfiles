#!/usr/bin/env bash
# Regression tests for agent/hooks/claude/swarm-fable-gate.sh (PreToolUse:Task|Agent ゲート)。
...
#   - マーカー無し / 該当 task の grant 無し / 期限切れ (TTL 600s) は stderr 平文 "FABLE_GATE: ..." + exit 2
#     (exit 2 時、Claude Codeはstdoutを無視しstderrのみ読むため、reasonは必ずstderrへ出力する)
...
# $here = agent/skills/swarm-implement/scripts。実hookは agent/hooks/claude/ 配下
...
HOOK="$(cd "$here/../../../hooks/claude" && pwd)/swarm-fable-gate.sh"
...
out=$(gate '{"tool_name":"Agent","tool_input":{"model":"fable","prompt":"[fable-spot:hk0] x"}}'); check "marked but no grant blocked" 2 $? 'FABLE_GATE:' "$out"
...
out=$(gate '{"tool_name":"Agent","tool_input":{"model":"fable","prompt":"[fable-spot:hk1] again"}}'); check "second spawn without new grant blocked" 2 $? 'FABLE_GATE:' "$out"
# 5. 他 task の grant は窃取できない（不一致マーカーはブロック・grant は生存）
bash "$SCRIPT" --fable hkA >/dev/null 2>&1
out=$(gate '{"tool_name":"Agent","tool_input":{"model":"fable","prompt":"[fable-spot:hkB] steal"}}'); check "mismatched marker blocked (no theft)" 2 $? 'FABLE_GATE:' "$out"
...
out=$(gate '{"tool_name":"Task","tool_input":{"model":"fable","prompt":"[fable-spot:hk9] x"}}'); check "Task tool name also gated" 2 $? 'FABLE_GATE:' "$out"
...
out=$(gate '{"tool_name":"Agent","tool_input":{"model":"fable","prompt":"[fable-spot:hk2] x"}}'); check "expired grant blocked" 2 $? 'FABLE_GATE:' "$out"
...
out=$(gate '{"tool_name":"Agent","tool_input":{"model":"fable","prompt":"[fable-spot:hk3] x"}}'); check "dot-prefixed stray never consumed as grant" 2 $? 'FABLE_GATE:' "$out"
...
check "conf-overridden TTL expires grant" 2 $? 'FABLE_GATE:' "$out"
