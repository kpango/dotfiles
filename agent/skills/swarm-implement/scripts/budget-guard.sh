...
if [ "${1:-}" = "--reset" ]; then
  ...
  shift
elif [ "${1:-}" = "--write-scope-grant" ]; then
  write_scope_grant_mode=true
  shift
fi
...
if $reset_mode; then
  ...
  rm -f "$dir/$sanitized_task" "$dir/.lock-$sanitized_task" "$dir/_fable-$sanitized_task" \
    "$dir/.lock-fable-$sanitized_task" "$dir/_fable-$sanitized_task-maker" "$dir/.lock-fable-$sanitized_task-maker"
...
if $write_scope_grant_mode; then
  lib="$(dirname "${BASH_SOURCE[0]}")/write-scope-lib.sh"
  # shellcheck disable=SC1090
  . "$lib"
  target="${positional[0]:?usage: budget-guard.sh --write-scope-grant <file-path>}"
  real=$(realpath -m "$target" 2>/dev/null || echo "$target")
  if ! write_scope_is_protected "$real"; then
    echo "NOT_PROTECTED: '$real' is not a Tier B write-scope path — no grant needed, Edit directly" >&2
    exit 1
  fi
  key=$(write_scope_key "$real")
  grants_dir="$dir/.write-scope-grants"
  mkdir -p "$grants_dir"
  find "$grants_dir" -maxdepth 1 -type f -mmin +"$(((WRITE_SCOPE_GRANT_TTL_SECONDS + 59) / 60))" -delete 2>/dev/null || true
  printf 'path=%s\ngranted_at=%s\n' "$real" "$(date -Is)" >"$grants_dir/$(date +%s%N)-$key"
  echo "write-scope grant issued: $key (ttl=${WRITE_SCOPE_GRANT_TTL_SECONDS}s)"
  exit 0
fi
...
if $fable_mode; then
  ...
  spot_log="$(swarm_state_dir)/fable-spot-log.jsonl"
...
# task カウンタの read-modify-write も flock で直列化する (Fable モードの task カウンタ
# (fd 201, .lock-fable-$sanitized_task) と同じ理由・同じパターン: ロック無しの cat→+1→printf
# は並行呼び出し(2 worktree 以上の並列実装、SWARM.md §4)でインクリメントが失われる、または
# 複数プロセスが同一 attempt 番号を重複して観測するレースコンディションになるため必須。
{
  flock -x 203
...
# SWARM.md §3 の3試行目ソフトチェックポイントを機械的にフラグする(既存は呼び出し元LLMの手動比較
# n==3 に依存する未機械化ギャップだった、SWARM.md §2 参照)。判定・分岐自体はしない(exit 0のまま)。
if [ "$n" -eq 3 ]; then
  echo "SOFT_CHECKPOINT_DUE: attempt 3/$max reached — 同じ仮説のまま試行4-5を消費する前にFixer切替を検討せよ(SWARM.md §3)"
fi
