...
> **Pi harness (default SKILL.state working pattern):** `skill_state_declare` domain
> `swarm-implement` with the schema in the `## SKILL.state Schema` section, then after
> each attempt/phase `skill_state_update` the changed keys. The auto-injected Σ (that
> section) is your working context. Pi-only default; inert on harnesses without the
> `skill_state_*` tools. Σ never overrides this skill's sources of truth
> (`budget-guard.sh`, the Checker `VERDICT`, the trajectory log + `@fix_plan.md`).
...
`tested_hypotheses` (append) models the paper's InterCode-CTF benefit (explicit
hypotheses stop repeated failed commands). The authoritative state remains this
skill's existing sources of truth — `budget-guard.sh` (attempt/budget counters),
the Checker's `VERDICT` line, and the trajectory log + `@fix_plan.md`; this Σ is
only a subordinate projection for the Pi runtime and never overrides them.
