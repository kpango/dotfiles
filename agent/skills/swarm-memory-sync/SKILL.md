...
description: >-
  ...
  を通じて軌跡ログ・`@fix_plan.md` に蓄積された「学び」のうち、一般化可能なものを
  `~/.claude/memory/`(auto-memory）へ蒸留するブリッジ。トリガー: `swarm-loop` Phase 5 GATE 完了時
  (軌跡ログ追記の直後、内部呼び出し)、`swarm-loop` Phase 2 PLAN の Interactive 設計インタビュー
  終了時(内部呼び出し)、または人間による `/swarm-memory-sync` の明示招集(既存軌跡ログの遡及処理・
...
`swarm-evolve`（SWARM.md §5、Skill 自体の行動規範を進化させる）とは軸が異なる（差分・境界条件は
frontmatter の description を参照）。本 skill は純粋なドメイン知識の記録であり、可逆かつ低リスク
（誤って書いても memory はいつでも Edit・削除できる）なため人間承認は不要 — 同じ理由から
`disable-model-invocation: false`（`swarm-evolve` は高警戒レベルのため `true` に限定しているのと対照的）
とし、`swarm-loop` Phase 2/5 からの内部呼び出しを自然文起動でも妨げないようにしている。

なお、Haiku 探索群・Maker・Checker 個々の生ログそのものを本 skill の入力にはしない。これらは
既に `swarm-secretary` の構造化レポートと `swarm-implement` の完了処理を経て軌跡ログ /
`@fix_plan.md` に要約済みであり、本 skill はその要約からさらに一般化可能な部分だけを抜き出す
（Observation Masking 原則、SWARM.md 不変条件を維持）。

## トリガーと入力ソース

1. **`swarm-loop` Phase 5 GATE からの内部呼び出し**（通常経路）: 今回のミッションの軌跡ログ
...
2. **`swarm-loop` Phase 2 PLAN（Interactive 設計インタビュー終了時）からの内部呼び出し**: この時点では
   軌跡ログ / `@fix_plan.md` の学びはまだ記入されていないため、入力は代わりに直近の対話ターン
...
4. **重複チェック（決定論的、必須）**（既存メモリの拡張を優先し、新規乱立を防ぐ）:
   ...
   - マッチがあれば当該ファイルを `Read` で確認し、同一トピックなら **新規ファイルを作らず** `Edit` で
     更新する(追記・訂正・古い情報の置き換え)。`memory-guard.sh` が「継続更新による劣化リスク
     （arXiv:2605.12978）」を警告した場合（同一ファイルへの`updated`記録が3回以上）は、既存文の書き換え
     ではなく新しい箇条書き・`## 追記`節としての**追加**を優先する（自由形式の全文書き換えを繰り返すと
     ground truthから drift するという知見への対処。§6 のデータの完全性原則と同型）。
...
5. **書き込み**:
   ...
   - 書き込み直後に `memory-guard.sh --record <file-name> <created|updated>` を呼ぶ（新規作成なら
     `created`、既存ファイルへのEditなら`updated`）。次回以降の重複チェック実行時に劣化リスク警告を
     出すための唯一の記録経路であり、省略すると手順4の警告が機能しない。
6. **完了報告** — 何件抽出し何件書いた（新規作成 / 既存更新の内訳）か、何件を「一般化不可」として
   却下したかを簡潔に報告する。却下理由の例も 1〜2 件添える(判定基準の透明性のため、軌跡ログの
   学びの3段階モデルにおける「機械化」の代わりに人間が判定基準を検証できるようにする)。

## 遡及処理(既存軌跡ログ全件からの移行)

人間による明示招集時、対象の軌跡ログ全体を読み、上記手順を 1 件ずつ適用する。既に
...
- SKILL.md / hooks / SWARM.md 自体の変更(境界条件は frontmatter の description 参照)
- 一般化不可な内容の記録(判定基準は手順 2 のグローバル CLAUDE.md 準拠基準に従う — memory 肥大化の防止)
...
- Haiku 探索群の生ログ・Maker/Checker の生の試行錯誤ログをそのまま転記すること(構造化レポート・
  最終的な学びのみを対象にする)
- `allowed-tools` の `Edit`/`Write` は `~/.claude/memory/` 配下への書き込みに機械的にスコープされて
  いない(`swarm-meta` の `harness-lint.sh` WRITE_SCOPE のような機械強制フックは本 skill には無い)。
  書き込み先を `~/.claude/memory/` 配下に限定するのは手順が定める運用上の制約であり、hook による
  実行時強制ではない — 逸脱しないことは実行者(Maker/人間)の責任で担保する
