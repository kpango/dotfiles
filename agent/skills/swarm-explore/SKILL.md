...
> `swarm-explore` with the schema in the `## SKILL.state Schema` section, then update
> the changed keys as shards complete. The auto-injected Σ (that section) is your
> working context. Pi-only default; inert on harnesses without the `skill_state_*`
> tools. Σ never overrides the secretary report (the authoritative aggregation).
...
The union/append fields model how concurrent fan-out shard findings combine under
the deterministic ⊕ merge. The secretary report remains the authoritative
aggregation of exploration results; this Σ is a subordinate projection for the Pi
runtime and never overrides that step.
