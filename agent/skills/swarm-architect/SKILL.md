...
- **Agent 召集ホワイトリスト**（「実装・修正」ではなく「分析・レビュー」が本分の agent に限る）:
  ...
  `agent/agents/*.md` に定義ファイルを持たない。呼び出し前に手元の Agent tool 一覧で実在を確認する） /
  `code-reviewer` / `security-audit` / `vald-reviewer` / 8 種の `*-adversarial-reviewer` 群
  （いずれも `tools` に Edit/Write を持たず `model: High` (Claudeでは `sonnet`) 固定 — 実装介入が構造的に不可能）/
  `perf-analyzer` / `ann-perf-engineer` / `ci-investigator`（この 3 つは `tools` に Edit/Write/Bash を
  持ち `model: inherit` — 下記「model 指定」の追加ルールが必須）。
  `go-expert`／`rust-expert`／`arch-ops`／`proto-expert`／`debugger`／`statusline-setup`／
  `python-expert`／`cpp-expert`／`k8s-expert`／`nix-expert`／`zig-expert`／`github-actions-expert`
  （実装・修正が本分）と `general-purpose`／`claude`（制限が無さすぎる）は対象外。
...
- **model 指定**: 召集する Agent/Skill には `model: 'Max'` / `model: 'fable'` を指定しない（既定の安価な階層を使う）。
  ...
  のため、`model` を省略すると自身（Fable/Max セッション）のモデルを暗黙継承しコストが跳ね上がる —
  SWARM.md §1 が Fixer(`debugger`) について既に定めた規則と同じ理由で、召集時は必ず
  `model: 'High'` (または `'sonnet'`) を明示する。
...
0. **起動前ゲート（呼び出し元が実行）** — `swarm-loop`（PLAN/CHECKPOINT）／`swarm-graph`（GRAPH-PLAN/REPLAN）
   ...
   本モードを `Agent(model: 'Max')`（Claude Codeでは `model: 'fable'` も可）で起動する際、prompt に
...
3. **Fable Maker への昇格判断**: 診断書の「実装介入の要否」が「必要」で、かつ高難易度ゲート
   ...
   `swarm-implement` が Maker を `model: 'Max'`（または `'fable'`）で起動してよい（同一スポット消費の継続、追加消費なし）。
   実装は本 skill の外で行われ、本 skill 自身は診断書提示で終了する（コード編集禁止は不変）。
   Fable Maker の成果物も通常の判定集約（Checker(XHigh)・並行レビュー・決定論的検証）を通す。
...
- DAG整合性チェック（依存順を確定させる前に自己点検し、結果を本セクションに並記する。いずれかが
  不成立の場合はタスク分割・依存順を修正してから提案書を確定する）:
  1. 型互換性: 各タスクの成果物が後続タスクの入力仕様と一致するか
  2. precondition充足: 各タスク開始に必要な前提条件が先行タスクの完了で満たされるか

## 難局突破の場合: 失敗軌跡の根本原因分析と、次の 5 試行で試すべき仮説の優先順位
```

DAG整合性チェックは `swarm-graph` の `graph-compile.sh` が担う CYCLE/MISSING_DEP の決定論的検証
（循環検出含む。宣言済みエッジの整合性のみを保証し、依存の書き忘れ自体は別途 `impact-scope.md` の
完全判定手順に拠る）とは対象が異なり、これを代替しない（SWARM.md §2「決定論的ツールを第一権威とする」
原則）。スポット診断モード（診断書フォーマット）には適用しない（最小限主義を維持するため）。
出典: `SWARM_REFERENCES.md`「swarm-architect 設計根拠」節。
