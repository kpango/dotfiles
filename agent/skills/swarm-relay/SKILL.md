...
- **SWARM.md §2 の既存 SendMessage 用法**（名前付き Agent 呼び出しの停滞対応）: `swarm-implement`/
  `swarm-evolve` が**同一セッション内**の Maker/Drafter 等（`name` 指定の Agent tool 呼び出し）に
  対し、`idle_notification` のみの停滞時に報告本文を再要求するために使う（`swarm-implement` の Checker
  は `Workflow` tool 起動のため本用法の対象外——詳細は `swarm-implement/SKILL.md`「idle_notification
  のみの停滞への対応」参照。`swarm-evolve` の Checker は対象内で変更なし）。宛先は自分がスポーンした
