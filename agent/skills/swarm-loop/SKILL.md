...
> **Pi harness (default SKILL.state working pattern):** at INIT `skill_state_declare`
> domain `swarm-loop` with the schema in the `## SKILL.state Schema` section, then
> after each phase `skill_state_update` the changed keys. The auto-injected Σ (that
> section) is your working context. Pi-only default; inert on harnesses without the
> `skill_state_*` tools. Σ never overrides `@fix_plan.md` (the authoritative progress
> state).
...
> **`run_adversarial_review`（pi ツール）は本 Phase の代替ではない（2026-09-07 追記）**: pi の
> `run_adversarial_review` / `/adversarial-review` は、diff に対する**軽量なヒューリスティック事前
> スクリーニング**（正規表現ベース、 6 レンズのみカバー。architecture / infra-config は非対応）であり、
> ここで定義する 8 Agent の敵対的レビュー（subagent がファイルを意味的に読む本番レビュー）の**代替では
> ない**。事前スクリーニングを通しても、本 Phase 4.5 の 8 Agent レビューは必ず実施する。
...
| 6   | `systems-lang-adversarial-reviewer` | Go/Rust/C++ 言語仕様             | `go-expert`/`rust-expert`/`cpp-expert`/`code-reviewer` と二段階防御                  |
| 7   | `shell-config-adversarial-reviewer` | Shell/ZSH/Makefile 言語仕様      | 新規領域                                                                             |
| 8   | `infra-config-adversarial-reviewer` | Nix/Lua/Yaml/Json 構文・スキーマ | Nixは`nix-expert`と二段階防御、Lua/Yaml/Jsonは新規領域（`vald-reviewer` とは非重複） |
...
--release-worktrees [--delete-branches]` で一括実行する（Option 1/4 のみ、必ずミッションworktree
   ...
   **ミッションworktree自体の統合・回収**（タスク単位worktree回収の**後**に実行 — ネストworktreeが
   ...
   danglingになる）:
   - Option 1（ローカルマージ）: `cd` でmain treeへ戻ったら、`cd` 自体の成否と
     `git branch --show-current` が `<base-branch>` であることを必ず確認してから（`cd` 失敗や
     ディレクトリ未遷移を無視すると、前のディレクトリ〈ミッションworktree外の想定外の場所〉に
     留まったまま人間承認前の内容がmainへ直接マージされる事故が実測されている——2026-08-12
     swarm-relayミッション参照）、ミッションブランチ（`swarm-<scale>/<slug>-<ts>`）を
     `<base-branch>` へマージしてから `worktree-release.sh <mission-worktree-path>`
     （ブランチ保持）でミッションworktreeを回収する。
...
SKILL.state (arXiv:2608.26263) execution-state model for this long-horizon procedural
skill. Under the Pi harness this is the DEFAULT working pattern: declare this schema
with `skill_state_declare` at the start of a run, then emit ΔΣ via `skill_state_update`
after each phase (null-deletion merge, runtime-validated with rollback-retry) instead of
re-deriving from accumulated history. The Pi `skill-state` extension also auto-injects a
bounded Σ digest into context every turn by default (`/state autocontext off` to
disable), so Σ — not the full transcript — is the sufficient statistic the model reasons
over. Per-turn prompt footprint stays bounded w.r.t. turn count (O(1) for replace-typed
fields; append/union fields grow with retained items and are the author's responsibility
to bound). On harnesses without the `skill_state` tools this section is inert
documentation. Σ is a subordinate projection and does NOT replace the skill's existing
source-of-truth state files. Reference: `agent/harnesses/pi/extensions/SKILL_STATE.md`.
...
```json
{
  "phase":        { "type": "string" },
  "scale":        { "type": "string" },
  "mission_slug": { "type": "string" },
  "tasks":        { "type": "map" },
  "blocked":      { "type": "list", "listMerge": "union" },
  "attempts":     { "type": "number" },
  "learnings":    { "type": "list", "listMerge": "append" }
}
```

`@fix_plan.md` remains the **sole authoritative progress state** (this skill's
existing invariant); this Σ is only a subordinate projection for the Pi runtime and
never overrides `@fix_plan.md`.
