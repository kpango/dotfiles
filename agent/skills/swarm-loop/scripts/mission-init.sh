...
. "$(dirname "$0")/../../swarm-implement/scripts/swarm-common-lib.sh"
...
# @fix_plan.md はミッションworktree内に作成するため worktree-local な swarm_repo_root を使う
# (下記 common_git_dir の共有 .git 解決とは役割が異なる)。
root=$(swarm_repo_root)
state="$(swarm_state_dir)/missions/$slug"
