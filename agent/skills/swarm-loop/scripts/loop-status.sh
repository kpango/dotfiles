...
# @fix_plan.md はミッションworktree内にあるため worktree-local な swarm_repo_root を使う。
...
budget_dir="$(swarm_state_dir)/budget"
...
git -C "$root" worktree list | grep -F "$root/.claude/worktrees/" || echo "  (なし)"
