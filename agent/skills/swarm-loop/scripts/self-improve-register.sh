...
# self-improve-check.shと同じ理由 — 2026-09-04、claude/pi/agy個別のself-improve-registry.tsvを
# agent/へ統合したことに伴う変更)。readlinkを独立したstatementにする理由もself-improve-check.sh
# と同じ(dirnameへの直接ネストはreadlink失敗をset -eから隠してしまう)。
