...
## モデル階層投資の指針（参考、単一preprint・adversarial検証未完了）

harness-updating能力（Drafter/Checkerが良い差分を生成・検証する能力）はモデル階級に対し比較的フラットで、
実行担当（`swarm-implement`のMaker/Checker、Fixer）への能力投資の方がタスク遂行成果への効き目が大きい
という報告がある（arXiv:2605.30621）。将来 Drafter/Checker のモデル階層を引き上げる提案が出た場合は、
同じ予算を `swarm-implement` 側に振り向ける代替案と比較検討すること。ただしこの知見は生成側（Drafter）
への追加投資が飽和しやすいことを示すのみで、検証側（Checker）を弱めてよい根拠にはならない — 生成能力の
飽和と検証能力の必要性は別問題であり、Drafter/Checkerの階層差（verifier独立性原則、SWARM.md §2）を
弱める理由には使わない。
...
- **proposed_diff の逐語自己検査（提出前必須、scope creep対策）**: 各提案の `new_string`/diff を
  証拠一式の引用箇所と自ら突き合わせ、(a) 許可範囲・閾値・主体・回数等の具体的主張で証拠に無い
  ものを追加していないか、(b)「既存の許可範囲は変更しない」等と自己申告する場合はproposed_diffが
  実際に新しい許可経路や閾値緩和を作っていないか、(c) 引用元ミッション名・件数が証拠の実数と
  一致するかを1つずつ確認する。この自己検査を経てもなお不確実な箇所があれば、提案から削るか
  `target` のスコープを狭める（それでも見逃した場合の最終防御は Checker の逐語照合に委ねる)。
  （根拠: 2026-07-13/07-17/2026-08-05/2026-09-03〈同日2件〉の計5件、Checkerが同一カテゴリ
  「evidence引用は逐語正確だが new_string に証拠にない規範緩和・主体・回数を混入」で却下——
  Drafter自身の自己申告〈diff_type・スコープ表明〉は信頼できないとCheckerが繰り返し指摘して
  いるため、Checker到達前の自己検査を追加する)。
