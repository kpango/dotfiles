...
- **2026-09-20 DeepResearch 追記**: Nine Judges の知見（上記）と同型の judge 間相関は、agent-swarm 文脈とは
  異なる設定（GRPO 型 RL 訓練の rollout group、Qwen2.5・約 25,000 completion group）でも独立に観測されている
  （arXiv:2609.06386、2026-09、cited-set より新しい単一プレプリント・未複製）。グループ内相関 ρ≈0.53 で
  8 completion の統計的重みは実効約 1.7 サンプル分。相関の駆動要因は判定対象の表層形式（分数・根号・記号
  表現等の一致しやすい形式的な難所）であり、モデル系列そのものではないと報告される。本基盤の Checker 設計
  （単一 Opus 一発判定）には直接の適用対象が無い（複数 Checker パネル化をそもそも採用していないため）が、
  「cross-family であること自体が独立性を保証する」という誤解を避けるための追加裏付けとして記録する。
