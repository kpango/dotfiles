...
- **`model_profiles`**: claude(Anthropic専用)・pi(複数provider対応)・agy(Google Gemini専用)は
  ...
  そのまま登録した(新規選定ではない追認)。各ツールの実 settings.json は現行モデル
  (claude=`opusplan`・pi=`anthropic/claude-sonnet-5`・agy=`gemini-3.8-flash-high`)を使用しており、
  `baseline` プロファイルもこれを追認した値へ同期済み(2026-09-05、`test-settings-common.sh` が
  実ファイルとの一致を検証)。
...
食い違っていた。pi には当時この種のPostToolUse lintフック自体が存在しなかった（2026-09-07の
pi-migration ミッションで `agent/harnesses/pi/extensions/post-edit-lint.ts` を新設し、agy版と同じ
broadest-safest-superset方針〈go/py/json/sh/yaml/toml、Makefileは `$(shell)` RCE罠のため意図的に
非対象〉で `pi.on("tool_result")` の write/edit に配線した。pi版はtool_resultが実行後に発火する
助言フックのため、agy版の `gofmt -s -w` によるその場整形ではなく `gofmt -e` の構文チェックのみ
〈ファイル無改変〉とする点が異なる）。
...
- **Pi の `dmi:true` 相当機構の欠如**: 上記調査結果参照。**訂正（2026-09-06〜07、
  pi-migration-audit ミッション）**: 上記調査結果が指す欠如は Agent 側（`subagents.ts` が起動する
  subagent の委譲制御）についてのものであり、Skill 側（`swarm-loop`/`swarm-graph`/`swarm-meta` が
  使う SKILL.md frontmatter の `disable-model-invocation`）は既に Pi Coding Agent 本体が
  ネイティブサポート済みと確認した（[pi.dev/docs/latest/skills](https://pi.dev/docs/latest/skills)
  で確認: true の場合 system prompt の `<available_skills>` から除外され `/skill:name` の明示
  呼び出しのみ受け付ける）。Agent 側の委譲制御フィールド欠如は引き続き残り、設計課題として残る
  （上記の Skill 側との区別を踏まえた上で、この Agent 側の委譲制御フィールド欠如は
  実装スコープ外の設計課題として切り分けている）。
