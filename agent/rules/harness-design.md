...
- `~/.claude/memory/` は Claude Code 自身のネイティブ機能であり、project 固有の値の書き場所として本行は
  そのまま有効（Pi/Claude/AGY の自前 SessionStart フックが自身の検索/注入に何をバックエンドとして使うか
  とは独立の話 — 現状は `agent/scripts/hooks/supermemory.sh` 経由）
