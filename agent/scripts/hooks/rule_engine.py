...
# ---------------------------------------------------------------------------
# write_scope: Tier B 書込保護ゲート。正典は
# agent/skills/swarm-implement/scripts/write-scope-lib.sh の write_scope_is_protected()
# (bash case文)。agent/write-scope-rules.jsonの$comment参照 — suffix_patterns/
# fixed_absolute_path_patterns の2配列ともfnmatchcase(bash case文と同じ大文字小文字区別あり
# のglob一致)で照合する。fixed_absolute_path_patternsは`${HOME}`プレースホルダを実際のhomeへ
# 置換してからfnmatchcaseする(worktree名を含む4件は`*`を含むglob一致、残り4件は結果的な
# 完全一致 — 完全一致もfnmatchcaseの特殊ケースとして同一コードパスで扱えるため分岐しない)。
# ---------------------------------------------------------------------------

def eval_write_scope(rules: dict, realpath: str, home: str) -> bool:
    """realpath解決済みの絶対パスがTier B保護対象か判定する。呼び出し側
    (decide.py handle_write_scope)がrealpathの解決(resolve_write_path_candidates)を担う。"""
    for pat in rules.get("suffix_patterns", []) or []:
        if fnmatch.fnmatchcase(realpath, pat):
            return True
    for pat in rules.get("fixed_absolute_path_patterns", []) or []:
        if fnmatch.fnmatchcase(realpath, pat.replace("${HOME}", home)):
            return True
    return False
