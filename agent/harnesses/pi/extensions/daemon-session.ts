...
export default function (pi: ExtensionAPI) {
  // Duplicate-run prevention: refuse to spawn a second daemon when a live
  // daemon with the identical objective already exists (workflow recovery
  // lens). Returns the existing record for reuse instead of double-spawning.
  const findRunningDaemonByObjective = (objective: string): DaemonSessionRecord | undefined => {
    const o = (objective || "").trim();
    if (!o) return undefined;
    // listActiveDaemons() already revalidates process liveness and flips
    // dead records to COMPLETED/FAILED, so filtering on RUNNING/STARTING is
    // exact: a past session with the same objective is never "reused".
    return listActiveDaemons().find(
      (d) => (d.objective || "").trim() === o && (d.status === "RUNNING" || d.status === "STARTING")
    );
  };
  ...
  const spawnOrReuse = (
    ...
    cwd: string
  ): { reused: boolean; record: DaemonSessionRecord } => {
    const existing = findRunningDaemonByObjective(objective);
    if (existing) return { reused: true, record: existing };
    return { reused: false, record: spawnDaemonProcess({ objective, cwd, model }) };
  };
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      try {
        ...
        const { reused, record } = spawnOrReuse(params.objective, params.model, cwd);

        if (reused) {
          ctx?.ui?.notify?.(
            `[Daemon] Reusing running daemon (${record.daemonId}, PID: ${record.pid}) with identical objective.`,
            "info"
          );
        } else {
          ctx?.ui?.notify?.(
  ...
  pi.registerCommand("daemon", {
    ...
    handler: async (args, ctx) => {
      ...
      if (action === "spawn") {
        ...
        try {
          const cwd = ctx?.cwd || process.cwd();
          const { reused, record } = spawnOrReuse(target, undefined, cwd);
          if (reused) {
            ctx.ui?.notify?.(
              `Daemon already running (reused):\n• ID: ${record.daemonId}\n• PID: ${record.pid}`,
              "info"
            );
            return;
          }
