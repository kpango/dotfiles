...
export async function executeMeshPublish(args: any, ctx?: any) {
  try {
...
export async function executeMeshQuery(args: any) {
  try {
    const stream = getOrCreateStream(args.correlationId);
    ...
    // Attach causal-DAG integrity metadata so query results surface dangling
    // parent references or cycles in the correlation.
    const integrity = validateCausalDagMessages(msgs);
          ...
          text: JSON.stringify(
            {
              ...
              dagValid: integrity.valid,
              danglingParents: integrity.danglingParents.slice(0, 10),
              cycles: integrity.cycles.slice(0, 10),
              messages: recent,
            },
            null,
            2
...
export async function executeMeshDagCheck(args: any) {
  try {
    const stream = getOrCreateStream(args.correlationId);
    const msgs = await stream.readAll();
    const integrity = validateCausalDagMessages(msgs);
    const text = integrity.valid
      ? `Causal DAG valid: ${msgs.length} messages, no dangling parents, no cycles.`
      : `Causal DAG issues (${msgs.length} messages):\n` +
        `  danglingParents (${integrity.danglingParents.length}): ${integrity.danglingParents.slice(0, 10).join("; ") || "-"}\n` +
        `  cycles (${integrity.cycles.length}): ${integrity.cycles.slice(0, 10).join("; ") || "-"}`;
    return { content: [{ type: "text", text }] };
  } catch (err: any) {
    return { content: [{ type: "text", text: `Error checking mesh DAG: ${err.message}` }] };
  }
}

export async function executeMeshStale(args: any) {
  try {
    const staleMs = parsePositiveIntArg(args.staleMs, 300_000);
    // Scope: correlation streams seen by this process via getOrCreateStream.
    // Persistent blackboard files from other sessions are not scanned here.
    const streams = activeStreams;
    const byCorr = new Map<string, MeshMessage[]>();
    for (const [, stream] of streams) {
      const msgs = await stream.readAll();
      for (const m of msgs) {
        if (!byCorr.has(m.correlationId)) byCorr.set(m.correlationId, []);
        byCorr.get(m.correlationId)!.push(m);
      }
    }
    const stalled = detectStalledHandoffsMessages(byCorr, staleMs);
    const text =
      stalled.length === 0
        ? `No stalled handoffs (threshold ${staleMs}ms).`
        : `Stalled handoffs (threshold ${staleMs}ms):\n` +
          stalled.map((s) => `  ${s.correlationId}: idle ${s.ageMs}ms, ${s.messageCount} messages`).join("\n");
    return { content: [{ type: "text", text }] };
  } catch (err: any) {
    return { content: [{ type: "text", text: `Error checking stalled handoffs: ${err.message}` }] };
  }
}

export async function executeMeshConfidence(args: any) {
  try {
...
export async function executeMeshHandoff(args: any) {
  try {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: any, _signal: any, _onUpdate: any, ctx: any) {
      return executeMeshPublish(params, ctx);
    },
    handler: async (args, ctx) => {
      return executeMeshPublish(args, ctx);
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: any, _signal: any, _onUpdate: any, _ctx: any) {
      return executeMeshQuery(params);
    },
    handler: async (args) => {
      return executeMeshQuery(args);
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: any, _signal: any, _onUpdate: any, _ctx: any) {
      return executeMeshConfidence(params);
    },
    handler: async (args) => {
      return executeMeshConfidence(args);
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: any, _signal: any, _onUpdate: any, _ctx: any) {
      return executeMeshHandoff(params);
    },
    handler: async (args) => {
      return executeMeshHandoff(args);
    },
  });
  ...
  pi.registerCommand("mesh", {
    description: "Inspect subagent mesh status, blackboard streams, causal DAG integrity, or reset state.",
    handler: async (args, ctx) => {
      ...
      if (subcmd === "dag") {
        const res = await executeMeshDagCheck({ correlationId });
        ctx.ui?.notify?.(res.content[0]?.text ?? "No DAG result", res.content[0]?.text?.startsWith("Causal DAG valid") ? "info" : "error");
        return;
      }

      if (subcmd === "stale") {
        const staleMs = parsePositiveIntArg(parts[1], 300_000);
        const res = await executeMeshStale({ staleMs });
        ctx.ui?.notify?.(res.content[0]?.text ?? "No stale result", "info");
        return;
      }
      ...
      ctx.ui?.notify?.(`Unknown mesh subcommand: ${subcmd}. Usage: /mesh [status|dag|stale|stream|reset] [correlationId]`, "error");
    },
  });
}
