...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const spec = synthesizeSkillFromTrace(trace, params.name);
      if (params.description) {
        spec.frontmatter.description = params.description;
      }

      const result = saveSkill(spec, { overwrite: params.overwrite ?? false });
