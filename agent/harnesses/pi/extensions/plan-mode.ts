...
// Safe read-only commands allowed in plan mode. NOTE: deliberately excludes
// `env` — `env <cmd>` is an arbitrary command runner (like sh -c / xargs) and
// would let any mutation through plan mode.
...
export function isSafeCommand(cmd: string): boolean {
  ...
  // Reject shell constructs that can execute or write ARBITRARY commands and
  // thereby escape the read-only allowlist entirely: command substitution
  // (`$(...)` / backticks), process substitution, redirections (`>`/`<`), and
  // embedded newlines (which the [;&|] split below does not catch). Without this
  // a "safe" prefix like `git status $(rm -rf x)` or `cat foo\nrm -rf x` would
  // pass. Be conservative: any of these => not safe.
  if (/\$\(|`|>|<|\n|\r/.test(trimmed)) {
    return false;
  }
  // Deny dangerous operators (chaining): every chained sub-command must be safe.
  ...
  // `find` is allowlisted for read-only traversal, but its -delete/-exec* family
  // turns it into a mutation/command runner; deny those explicitly.
  if ((trimmed === "find" || trimmed.startsWith("find ")) &&
      /\s-(delete|exec|execdir|ok|okdir|fprint|fprintf|fprint0)\b/.test(trimmed)) {
    return false;
  }
  // `git branch` is allowlisted for listing, but its mutating flags
  // (-d/-D/-m/-M/-c/-C/-f and their long forms) delete, rename, copy, or
  // force-move branches — real mutations that must not run in read-only plan
  // mode. Read-only listing flags (-a/-r/-v/-l/--contains/--merged/...) are not
  // matched.
  if ((trimmed === "git branch" || trimmed.startsWith("git branch ")) &&
      /\s(-[dDmMcC]|-f\b|--(delete|move|copy|force|edit-description|set-upstream-to|unset-upstream))/.test(trimmed)) {
    return false;
  }
  // Prefix match with a word boundary so `cat`/`ls` do not match `category`/`lsof`.
  return SAFE_COMMAND_PREFIXES.some(
    (safe) =>
      trimmed === safe ||
      trimmed.startsWith(`${safe} `) ||
      trimmed === `rtk ${safe}` ||
      trimmed.startsWith(`rtk ${safe} `)
  );
}
