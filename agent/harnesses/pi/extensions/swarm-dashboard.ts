...
export function renderDashboardText(summary: FixPlanSummary, worktrees: string[]): string {
  ...
  const barLen = 20;
  // Clamp to [0, barLen] so a malformed summary (completedTasks > totalTasks,
  // negative, or NaN) can never pass a negative count to String.repeat, which
  // throws RangeError and breaks the whole render.
