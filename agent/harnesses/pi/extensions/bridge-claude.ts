...
/**
 * Build the `claude` CLI argv. The prompt is placed AFTER a `--`
 * end-of-options separator so a prompt that begins with `-`/`--` is treated as
 * the positional prompt rather than misparsed as a CLI flag (verified: `claude
 * --print "--foo"` errors with `unknown option`, while `claude --print -- "--foo"`
 * accepts it).
 */
