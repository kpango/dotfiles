...
function tryGraphifyCli(subcommand: string, args: string[], cwd: string): string | null {
  try {
    // Pass the subcommand and user-provided args as a literal argv array via
    // spawnSync (no shell), so a query/concept containing shell metacharacters
    // ($(...), backticks, ;, |) is treated as a plain search term and cannot
    // inject commands. The prior execSync string only escaped double quotes,
    // leaving $(...) and backtick command substitution exploitable.
    const res = spawnSync("graphify", [subcommand, ...args], {
      cwd,
      encoding: "utf-8",
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      const result = queryGraph(params.query, ctx.cwd);
      return {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      const result = explainConcept(params.concept, ctx.cwd);
      return {
