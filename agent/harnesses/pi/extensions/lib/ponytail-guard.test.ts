...
// --------------------------------------------------------------------------
// L1 regression: auditDiff must detect a two-line empty catch added in a diff
// (the closing brace line is "+}"; its diff prefix must be stripped first).
check("L1: auditDiff detects a two-line empty catch block",
  auditDiff("+++ b/x.ts\n@@\n+try { foo(); } catch (e) {\n+}\n+bar();").safetyFindings.some((f) => f.message.includes("empty catch")));
check("L1: auditDiff does NOT flag a non-empty catch",
  !auditDiff("+++ b/x.ts\n+try{f()}catch(e){\n+  log(e);\n+}").safetyFindings.some((f) => f.message.includes("empty catch")));

// L2 regression: auditDiff must flag require()-style bloat imports, not just
// `import ... from`.
check("L2: auditDiff flags require('lodash')",
  auditDiff('+++ b/x.js\n+const _ = require("lodash");').bloatFindings.some((f) => f.message.includes("external dependency")));
check("L2: auditDiff still flags import from 'lodash'",
  auditDiff('+++ b/x.ts\n+import _ from "lodash";').bloatFindings.some((f) => f.message.includes("external dependency")));
check("L2: auditDiff does NOT flag an allowed require('fs')",
  !auditDiff('+++ b/x.js\n+const fs = require("fs");').bloatFindings.some((f) => f.message.includes("external dependency")));

// L2 regression: the python empty-except detector must stay linear (no ReDoS)
// and still flag `except: pass`.
check("L2: auditCode still flags an empty except: pass",
  auditCode("try:\n    x()\nexcept:\n    pass\n", "python").safetyFindings.some((f) => f.type === "error_suppression"));
check("L2: auditCode does NOT flag 'exceptional:' (word boundary preserved)",
  !auditCode("exceptional: pass\n", "python").safetyFindings.some((f) => f.type === "error_suppression"));
{
  const t0 = performance.now();
  auditCode("except:\n" + "\n".repeat(100000), "python");
  const elapsed = performance.now() - t0;
  check("L2: pathological 'except:' + 100k newlines audits in < 500ms (no ReDoS)", elapsed < 500, `${elapsed.toFixed(1)}ms`);
}
