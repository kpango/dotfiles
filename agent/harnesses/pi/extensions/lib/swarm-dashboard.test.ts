...
// L2 regression: the progress bar must never crash on a malformed summary where
// completedTasks > totalTasks (would send a negative count to String.repeat).
{
  let threw = false;
  let out = "";
  try {
    out = renderDashboardText({ totalTasks: 4, completedTasks: 5, tasks: [] } as FixPlanSummary, []);
  } catch {
    threw = true;
  }
  check("L2: overflow summary (5/4) does not throw", !threw && out.includes("Progress"));
}
