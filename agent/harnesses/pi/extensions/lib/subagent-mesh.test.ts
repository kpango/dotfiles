...
import {
  ...
  createMeshMessage,
  validateCausalDagMessages,
  detectStalledHandoffsMessages,
  parsePositiveIntArg,
...
async function runTier1FeatureTests() {
  ...
  console.log("\n[Feature 2: Blackboard File Stream]");
  {
    ...
    try {
      ...
      // NOTE: the single-shot fallback of BlackboardStream.append (fellBack guard)
      // is exercised via a standalone probe rather than here. A prior version of
      // this test forced the flock spawn to fail by mutating the GLOBAL
      // process.env.PATH, which raced with git-using tests in other files under
      // bun's parallel file runner and made the suite flaky. Global-env mutation
      // in a unit test is unsafe when the runner parallelizes files.
  ...
  console.log("\n[Feature 3: Coordinator Engine]");
  {
    ...
    // T1.3.3b: L2 regression — a hash re-seen AFTER its TTL expires must not push a
    // duplicate hashQueue entry (seenHashes/hashQueue must stay in 1:1 sync), else
    // a later eviction would drop a still-tracked hash.
    const ttlCoord = new CoordinatorEngine({ deduplicationWindowSize: 100, deduplicationTtlMs: 8 });
    const mSame = createMeshMessage({ topic: "spec", sender: "s", correlationId: "c", payload: { same: 1 } });
    ttlCoord.processMessage(mSame);
    await new Promise((r) => setTimeout(r, 20)); // exceed the 8ms TTL
    const reSeen = ttlCoord.processMessage(mSame);
    const sh = (ttlCoord as unknown as { seenHashes: Map<string, number> }).seenHashes;
    const hq = (ttlCoord as unknown as { hashQueue: string[] }).hashQueue;
    check("T1.3.3b: TTL re-seen message is accepted (not a within-TTL duplicate)", reSeen.accepted && !reSeen.duplicate);
    check("T1.3.3b: seenHashes and hashQueue stay in sync after TTL re-seen (no desync)",
      sh.size === 1 && hq.length === 1 && hq.length === new Set(hq).size);
    ...
    // T1.3.7: L2 regression — getActiveState().attempt reflects the number of
    // draft submissions (each remediation resubmits a draft), not a hardcoded 1.
    const attemptCoord = new CoordinatorEngine();
    const draft = (patch: string) => createMeshMessage({
      topic: "draft", sender: "worker", correlationId: "attempt-c",
      payload: { taskSlug: "auth", patch } as DraftPayload,
    });
    attemptCoord.processMessage(draft("v1"));
    check("T1.3.7: attempt is 1 after the first draft", attemptCoord.getActiveState("attempt-c").attempt === 1);
    attemptCoord.processMessage(draft("v2"));
    check("T1.3.7: attempt is 2 after a second (distinct) draft", attemptCoord.getActiveState("attempt-c").attempt === 2);
    check("T1.3.7: attempt is 1 (floor) for an unknown correlation", attemptCoord.getActiveState("none").attempt === 1);
  }
...
async function runTier5DagIntegrityTests() {
  // --- validateCausalDagMessages: dangling references ---
  {
    const msgs: MeshMessage[] = [
      {
        id: "m1",
        correlationId: "c1",
        parentMessageId: "missing-0",
        timestamp: 1,
        sender: { id: "a", role: "reviewer" },
        topic: "draft",
        payload: {},
      },
      {
        ...
        correlationId: "c1",
        timestamp: 2,
        sender: { id: "b", role: "checker" },
        topic: "verification",
        payload: {},
      },
    ];
    const r = validateCausalDagMessages(msgs);
    check("dag: dangling parent flagged", r.valid === false && r.danglingParents.length === 1);
  }

  // --- validateCausalDagMessages: simple chain is valid ---
  {
    const msgs: MeshMessage[] = [
      {
        id: "m1",
    ...
    const r = validateCausalDagMessages(msgs);
    check("dag: simple child->parent chain valid", r.valid === true && r.cycles.length === 0);
  }

  // --- validateCausalDagMessages: 2-node cycle reported exactly once ---
  {
    const msgs: MeshMessage[] = [
      {
        id: "A",
    ...
    const r = validateCausalDagMessages(msgs);
    check(
      "dag: 2-node cycle detected with exactly 1 report",
      r.valid === false && r.cycles.length === 1,
      `cycles=${r.cycles.join(";")}`
    );
  }

  // --- validateCausalDagMessages: self-loop detected once ---
  {
    const msgs: MeshMessage[] = [
      {
        id: "S",
    ...
    const r = validateCausalDagMessages(msgs);
    check("dag: self-loop detected exactly once", r.valid === false && r.cycles.length === 1);
  }

  // --- detectStalledHandoffsMessages: active vs stale ---
  {
    const now = 1_000_000;
    const active: MeshMessage[] = [
      {
        id: "m1",
        correlationId: "c5",
        timestamp: now - 10_000, // 10s old, below default 300s threshold
        sender: { id: "a", role: "maker" },
        topic: "draft",
        payload: {},
      },
    ...
    const stale: MeshMessage[] = [
      {
        id: "m2",
        correlationId: "c6",
        timestamp: now - 400_000, // > 5 min
        sender: { id: "b", role: "maker" },
        topic: "draft",
        payload: {},
      },
    ];
    const map = new Map<string, MeshMessage[]>([["c5", active], ["c6", stale]]);
    const stalled = detectStalledHandoffsMessages(map, 300_000, now);
    check(
      "stale: only the >5min correlation is flagged",
      stalled.length === 1 && stalled[0].correlationId === "c6",
      stalled.map((s) => s.correlationId).join(",")
    );
  }

  // --- L1: parsePositiveIntArg guards a NaN threshold (which would silently
  //     disable stall detection since `age > NaN` is always false) ---
  check("L1: parsePositiveIntArg('abc') falls back to default", parsePositiveIntArg("abc", 300_000) === 300_000);
  check("L1: parsePositiveIntArg(undefined) falls back to default", parsePositiveIntArg(undefined, 300_000) === 300_000);
  check("L1: parsePositiveIntArg('5000') parses", parsePositiveIntArg("5000", 300_000) === 5000);
  check("L1: parsePositiveIntArg(0) falls back (non-positive)", parsePositiveIntArg(0, 300_000) === 300_000);
  check("L1: parsePositiveIntArg(-5) falls back (non-positive)", parsePositiveIntArg(-5, 300_000) === 300_000);
  {
    const now = Date.now();
    const map = new Map<string, MeshMessage[]>([["c9", [
      { id: "m1", topic: "draft", correlationId: "c9", timestamp: now - 600_000, payload: {}, sender: { id: "a", role: "worker" } } as any,
    ]]]);
    const withGuard = detectStalledHandoffsMessages(map, parsePositiveIntArg("abc", 300_000), now);
    check("L1: a NaN threshold no longer hides a real stall (guarded to default)", withGuard.length === 1);
  }

  // --- detectStalledHandoffsMessages: terminal verification topic is not stalled ---
  {
    const now = 1_000_000;
    const done: MeshMessage[] = [
      {
        id: "m3",
        correlationId: "c7",
    ...
    const map = new Map<string, MeshMessage[]>([["c7", done]]);
    const stalled = detectStalledHandoffsMessages(map, 300_000, now);
    check("stale: terminal verification topic excluded", stalled.length === 0);
  }

  // --- L2 regression: activity age uses the NEWEST timestamp, not the last
  // appended, so an out-of-order recent message keeps the correlation active ---
  {
    const now = 1_000_000;
    const outOfOrder: MeshMessage[] = [
      { id: "a1", correlationId: "c8", timestamp: now - 10_000, sender: { id: "x", role: "worker" }, topic: "draft", payload: {} },
      // last APPENDED is old, but an earlier-listed message is recent:
      { id: "a2", correlationId: "c8", timestamp: now - 600_000, sender: { id: "x", role: "worker" }, topic: "critique", payload: {} },
    ];
    const map = new Map<string, MeshMessage[]>([["c8", outOfOrder]]);
    const stalled = detectStalledHandoffsMessages(map, 300_000, now);
    check("L2: out-of-order recent activity is NOT flagged stale (max timestamp)", stalled.length === 0);
    // And a genuinely old correlation (all timestamps old) IS flagged.
    const allOld: MeshMessage[] = [
      { id: "b1", correlationId: "c9", timestamp: now - 700_000, sender: { id: "x", role: "worker" }, topic: "draft", payload: {} },
      { id: "b2", correlationId: "c9", timestamp: now - 600_000, sender: { id: "x", role: "worker" }, topic: "critique", payload: {} },
    ];
    const stalled2 = detectStalledHandoffsMessages(new Map([["c9", allOld]]), 300_000, now);
    check("L2: genuinely old correlation IS flagged stale, age from newest", stalled2.length === 1 && stalled2[0].ageMs === 600_000);
  }
}
...
async function runBenchmarkTests() {
  ...
  console.log("\n[Benchmark 1: In-Memory Pub/Sub Dispatch Latency]");
  {
    ...
    // L2 regression: in-memory history is bounded to the newest MESH_HISTORY_MAX
    // messages so a long-running session cannot leak memory.
    {
      const bounded = new SubagentMesh();
      for (let i = 0; i < MESH_HISTORY_MAX + 500; i++) {
        bounded.publish({ id: String(i), topic: "spec", correlationId: "c", payload: {}, timestamp: i } as any);
      }
      const h = bounded.getHistory();
      check("L2: history is capped at MESH_HISTORY_MAX", h.length === MESH_HISTORY_MAX);
      check("L2: history keeps the newest message", h[h.length - 1].id === String(MESH_HISTORY_MAX + 499));
      check("L2: history drops the oldest messages", h[0].id === String(500));
    }

    // L1 regression: percentile indices must stay in-bounds and keep p95/p99
    // distinct from max (the old `floor(n*p)` collapsed p99 onto max for n<=100).
    {
      const seq = Array.from({ length: 100 }, (_, i) => i + 1);
      const pc = calculatePercentiles(seq);
      check("L1: percentile p50 is the lower-median (50)", pc.p50Ms === 50, `p50=${pc.p50Ms}`);
      check("L1: percentile p99 stays distinct from max", pc.p99Ms < pc.maxMs && pc.p99Ms === 99, `p99=${pc.p99Ms} max=${pc.maxMs}`);
      check("L1: percentile p95 stays distinct from max", pc.p95Ms === 95, `p95=${pc.p95Ms}`);
      const one = calculatePercentiles([42]);
      check("L1: single-sample percentiles are in-bounds", one.p99Ms === 42 && one.maxMs === 42);
    }
...
async function main() {
  ...
  await runTier5DagIntegrityTests();
