...
// L2 regression: judge must NOT falsely fire on the substring "ok" inside failure words.
check("judge: does NOT match 'broken' (substring ok)", !evaluateGoalPredicate("judge:x", { output: "the build is broken" }));
check("judge: does NOT match 'token limit exceeded' (substring ok)", !evaluateGoalPredicate("judge:x", { output: "token limit exceeded" }));
check("judge: does NOT match 'lookup failed' (substring ok)", !evaluateGoalPredicate("judge:x", { output: "lookup failed" }));
check("judge: still matches standalone 'OK'", evaluateGoalPredicate("judge:x", { output: "result is OK" }));
check("judge: still matches 'passed'", evaluateGoalPredicate("judge:x", { output: "all tests passed" }));
check("judge: still matches 'successful'", evaluateGoalPredicate("judge:x", { output: "deploy successful" }));
check("judge: still matches 'complete'/'completed'", evaluateGoalPredicate("judge:x", { output: "migration complete" }));
...
// L1 regression: a directly-constructed regex condition with an invalid pattern
// string must not crash evaluateGoalPredicate (parse-time validation is bypassed).
{
  let threw = false;
  let result = true;
  try {
    result = evaluateGoalPredicate({ type: "regex", raw: "regex:[", pattern: "[" } as any, { output: "x", exitCode: 0 });
  } catch {
    threw = true;
  }
  check("L1: invalid regex pattern string does not crash evaluateGoalPredicate", !threw && result === false);
  check("L1: valid regex string pattern still matches",
    evaluateGoalPredicate({ type: "regex", raw: "regex:ok", pattern: "ok" } as any, { output: "ok", exitCode: 0 }) === true);
  check("L1: pre-compiled RegExp pattern still matches",
    evaluateGoalPredicate({ type: "regex", raw: "regex:\\d+", pattern: /\d+/ } as any, { output: "n42", exitCode: 0 }) === true);
}
