...
export function auditCode(code: string, language?: string, filename?: string): AuditResult {
  ...
  if (lang === "python") {
    // Linear form: a single unambiguous `[^:\n]*` for the exception clause and a
    // comment-line repetition anchored by `#`. The prior
    // `(?:\s+[^:]+)?:\s*(?:#[^\n]*\n\s*|\n\s*|\s+)` overlapped whitespace matchers
    // and backtracked catastrophically (seconds) on `except:` + long runs of
    // newlines/spaces with no following `pass`.
    const pyPassRegex = /except\b[^:\n]*:\s*(?:#[^\n]*\n\s*)*pass\b/g;
    let matchPy;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("+")) {
      ...
      if (/catch\s*(\([^)]*\))?\s*\{\s*\}/.test(trimmed) || /catch\s*(?:\([^)]*\))?\s*\{/.test(trimmed)) {
        // Also check if next line or this line is empty catch. Strip the diff
        // prefix (+/-/space) from the next line first — an added closing brace is
        // "+}", whose raw form does not startsWith("}"), so a two-line empty catch
        // would otherwise be missed (auditCode catches it via a multi-line regex).
      ...
      // Check for bloated dependency imports introduced in diff. Allow an
      // optional `(` after the keyword so `require("lodash")` (paren before the
      // quote) is flagged like `import ... from "lodash"`.
      if (/(?:import|from|require)\s*\(?\s*["'](?:lodash|moment|axios)["']/.test(trimmed)) {
        bloatFindings.push({
          step: 3,
