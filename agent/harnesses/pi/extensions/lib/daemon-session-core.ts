...
export function spawnDaemonProcess(
  ...
  if (options.spawnFn) {
    ...
    // Fall back to 0 (an invalid pid) when the child reports none, matching the
    // real-spawn path below. Falling back to process.pid would alias the PARENT
    // Pi process: checkDaemonLiveness would then falsely report the daemon as
    // RUNNING (the parent is always alive), reuse-dedup would match a
    // never-started daemon, and a kill on record.pid would target the parent.
