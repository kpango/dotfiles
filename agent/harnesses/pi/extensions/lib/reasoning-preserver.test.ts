...
// L1 regression: targetTokenLimit must actually bound the summary body (was a dead param).
const manyKeyLines = Array.from({ length: 40 }, (_, i) => `Step: perform action number ${i} with detailed rationale text`).join("\n");
const budgeted = compressReasoningTrace(manyKeyLines, 20); // 20 tokens ~= 80 chars
const bodyMatch = budgeted.match(/<think-summary tokens="(\d+)">\n([\s\S]*)\n<\/think-summary>/);
check("compressReasoningTrace enforces targetTokenLimit (body <= budget*4 chars)",
  bodyMatch !== null && bodyMatch[2].length <= 20 * 4);
check("compressReasoningTrace reports token estimate within budget",
  bodyMatch !== null && parseInt(bodyMatch[1], 10) <= 20);
check("compressReasoningTrace still fits a small trace under budget without truncation marker",
  !compressReasoningTrace("Conclusion: done cleanly.", 1024).includes("[truncated to token budget]"));
