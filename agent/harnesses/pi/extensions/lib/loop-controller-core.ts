...
export function evaluateGoalPredicate(
  ...
  switch (condition.type) {
    ...
    case "regex": {
      if (!condition.pattern) return false;
      // A GoalCondition can be constructed directly (not via parseGoalCondition,
      // which already validates), so an invalid pattern string must not crash the
      // loop tick. Match the `eval` case: treat an invalid pattern as unsatisfied.
      let reg: RegExp;
      try {
        reg = typeof condition.pattern === "string" ? new RegExp(condition.pattern) : condition.pattern;
      } catch {
        return false;
      }
    ...
    case "judge": {
      // Word-boundary-anchored success detection. A plain substring match on
      // "ok" falsely fires on failure words that merely CONTAIN it ("br(ok)en",
      // "t(ok)en limit exceeded", "lo(ok)up failed"), terminating the loop as
      // "satisfied" on a failing turn. Requiring the success stem to start at a
      // word boundary (while still allowing suffixes like "passed"/"successful"/
      // "completed") eliminates the false positives without losing real signals.
      return /\b(pass|success|ok|complet)/i.test(outputText);
    }
    default:
      return false;
  }
