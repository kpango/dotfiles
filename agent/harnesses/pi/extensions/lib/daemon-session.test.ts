...
try {
  ...
  // L2 regression: a spawnFn that returns NO pid must fall back to 0 (invalid),
  // NOT process.pid — otherwise the record aliases the parent and
  // checkDaemonLiveness would falsely report the daemon alive.
  const pidlessSpawnFn = (_cmd: string, _args: string[], opts: any) => {
    fs.writeFileSync(opts.logPath, "pidless\n", "utf-8");
    return { unref: () => {} };
  };
  const pidlessRecord = spawnDaemonProcess({
    objective: "pidless daemon",
    cwd: "/workspace/project",
    daemonDir: testTempDir,
    spawnFn: pidlessSpawnFn,
  });
  eq("pidless spawnFn falls back to pid 0 (not process.pid)", pidlessRecord.pid, 0);
  check("pidless daemon is NOT reported alive (would alias parent if process.pid)",
    checkDaemonLiveness(pidlessRecord.pid) === false);
