...
export function compressReasoningTrace(fullTrace: string, targetTokenLimit = 1024): string {
  ...
  // Enforce the caller-supplied token budget. Previously targetTokenLimit was a
  // dead parameter: the summary was fixed at <=10 key lines (or head/tail) and
  // could silently exceed config.targetSummaryTokens when lines were long, so
  // compaction did not honor its own budget contract. estimateTokens is
  // ceil(len/4), so cap the body at targetTokenLimit*4 characters.
  if (targetTokenLimit > 0 && estimateTokens(summaryBody) > targetTokenLimit) {
    const maxChars = targetTokenLimit * 4;
    if (summaryBody.length > maxChars) {
      const marker = "\n... [truncated to token budget] ...";
      const keep = Math.max(0, maxChars - marker.length);
      summaryBody = summaryBody.slice(0, keep).trimEnd() + marker;
    }
  }
