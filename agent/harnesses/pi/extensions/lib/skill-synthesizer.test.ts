...
// L2 regression: a description with newlines/colon/# must not break the YAML
// frontmatter (no rogue key lines); simple values stay unquoted.
{
  const fmLines = (md: string) => md.split("---")[1].trim().split("\n");
  const mdNewline = formatSkillMarkdown({ frontmatter: { name: "my-skill", description: "Line one\nLine two: colon" }, body: "" } as any);
  const rogue = fmLines(mdNewline).filter((l) => !/^(name|description):/.test(l));
  check("L2: newline in description does not create rogue frontmatter lines", rogue.length === 0);
  check("L2: multiline description is collapsed onto the description line",
    fmLines(mdNewline).some((l) => l.startsWith("description:") && l.includes("Line one Line two")));
  const mdColon = formatSkillMarkdown({ frontmatter: { name: "s2", description: "Handles X: does Y # note" }, body: "" } as any);
  check("L2: colon/# description is double-quoted",
    fmLines(mdColon).some((l) => l === 'description: "Handles X: does Y # note"'));
  check("toYamlScalar: simple value stays unquoted", toYamlScalar("simple here") === "simple here");
  check("toYamlScalar: empty value is quoted", toYamlScalar("") === '""');
  check("toYamlScalar: quotes and backslashes are escaped",
    toYamlScalar('a "b" c') === '"a \\"b\\" c"');
}
