...
import { deriveCliBridgeOutput, runCliBridge, getCircuitBreakerState, resetCircuitBreakers, CIRCUIT_BREAKER_EXIT_CODE } from "./cli-bridge";
...
async function main() {
  ...
  // 6. timeout — 協調的プロセス(timeoutMs 500)は exitCode 124 / timedOut=true を返し、
  //    直後に close がクリーンなコードで発火してもタイムアウト結果を優先する。
  resetCircuitBreakers();
  const slowBin = writeFixture("slow.sh", "#!/usr/bin/env bash\nsleep 30\n");
  const start6 = Date.now();
  const r6 = await runCliBridge({
    binary: slowBin,
    args: [],
    cwd: tmpDir,
    timeoutMs: 500,
  ...
  check("timeout: exitCode=124", r6.exitCode === 124, `exitCode=${r6.exitCode}`);
  check("timeout: timedOut=true", r6.timedOut === true);
  check("timeout: resolves within ~5s", elapsed6 < 5000, `elapsed=${elapsed6}ms`);
  check("timeout: breaker counts a failure", getCircuitBreakerState(slowBin).failureCount === 1);

  // 7. circuit breaker — 3連続失敗で OPEN になり、後続呼び出しは即時 111 を返す。
  resetCircuitBreakers();
  const missing = path.join(tmpDir, "definitely-missing-bin");
  const r7a = await runCliBridge({ binary: missing, args: [], cwd: tmpDir, runningPlaceholder: "(running...)" });
  const r7b = await runCliBridge({ binary: missing, args: [], cwd: tmpDir, runningPlaceholder: "(running...)" });
  const r7c = await runCliBridge({ binary: missing, args: [], cwd: tmpDir, runningPlaceholder: "(running...)" });
  check("breaker: 3rd consecutive failure trips OPEN", getCircuitBreakerState(missing).status === "OPEN");
  const r7d = await runCliBridge({ binary: missing, args: [], cwd: tmpDir, runningPlaceholder: "(running...)" });
  check(
    "breaker: OPEN short-circuits with exitCode 111",
    r7d.exitCode === CIRCUIT_BREAKER_EXIT_CODE && r7a.exitCode === 1 && r7b.exitCode === 1 && r7c.exitCode === 1,
    `codes=${r7a.exitCode}/${r7b.exitCode}/${r7c.exitCode}/${r7d.exitCode}`
  );

  // 8. circuit breaker — 成功で CLOSED にリセットされる。
  resetCircuitBreakers();
  await runCliBridge({ binary: "definitely-missing-x", args: [], cwd: tmpDir, runningPlaceholder: "(running...)" });
  await runCliBridge({ binary: "definitely-missing-x", args: [], cwd: tmpDir, runningPlaceholder: "(running...)" });
  const okBin8 = writeFixture("ok8.sh", "#!/usr/bin/env bash\nexit 0\n");
  const r8ok = await runCliBridge({
    binary: okBin8,
    args: [],
    cwd: tmpDir,
    runningPlaceholder: "(running...)",
  });
  check("breaker: success resets state to CLOSED", r8ok.exitCode === 0 && getCircuitBreakerState(okBin8).failureCount === 0);
