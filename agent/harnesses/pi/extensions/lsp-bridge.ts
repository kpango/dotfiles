...
export function runSymbols(targetPath: string, cwd: string): string[] {
  ...
  if (fs.existsSync(fullPath)) {
    try {
      ...
      // Every alternative is anchored at ^ (the line is trimmed) so a func/fn/def
      // keyword appearing MID-LINE (in a comment or inside an identifier like
      // `myfunc`) is not misreported as a symbol. Leading language modifiers
      // (export/default/async for JS, pub/async/unsafe/const/extern for Rust,
      // async for Python, Go receivers) are allowed so real declarations still
      // match.
      const funcRegex =
        /^(?:export\s+)?(?:default\s+)?(?:async\s+)?function\s+([a-zA-Z0-9_]+)|^func\s+(?:\([^)]+\)\s+)?([a-zA-Z0-9_]+)|^(?:pub(?:\([^)]*\))?\s+|async\s+|unsafe\s+|const\s+|extern\s+"[^"]*"\s+)*fn\s+([a-zA-Z0-9_]+)|^(?:async\s+)?def\s+([a-zA-Z0-9_]+)/;
      const typeRegex =
        /^(?:export\s+)?(?:pub(?:\([^)]*\))?\s+)?(?:type|interface|class|struct|enum)\s+([a-zA-Z0-9_]+)/;
