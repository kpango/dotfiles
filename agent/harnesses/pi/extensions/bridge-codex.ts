...
const CodexParams = Type.Object({
  ...
  timeout: Type.Optional(Type.Integer({ description: "Execution timeout in seconds (default: 180s)", default: 180, minimum: 1 })),
});
...
/**
 * Build the `codex exec` argv. The prompt is placed AFTER a `--`
 * end-of-options separator so a prompt beginning with `-`/`--` is treated as the
 * positional prompt, not misparsed as a flag (verified: `codex exec "--foo"`
 * errors `unexpected argument` and codex itself tips to use `--`).
 */
export function buildCodexArgs(
  params: { prompt: string; model?: string; sandbox?: string; search?: boolean },
  targetCwd: string
): string[] {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      const args = buildCodexArgs(params, targetCwd);

      const { exitCode, stdout, stderr, wasAborted, timedOut } = await runCliBridge({
        binary: "codex",
        args,
        cwd: targetCwd,
        env: { ...process.env },
        signal,
        timeoutMs: (params.timeout ?? 180) * 1000,
