...
echo
echo "[ Extension Tool Contracts (execute: implementation) ]"
CONTRACT_ERRORS=0
for ext in "${EXTENSIONS[@]}"; do
    ext_path="$SCRIPT_DIR/extensions/$ext"
    for moved in "${MOVED_TO_AGENT_HOOKS[@]}"; do
    ...
    [[ -f "$ext_path" ]] || continue
    broken_tools=$(python3 -c "
import sys, re
with open('$ext_path', 'r', encoding='utf-8') as f:
    c = f.read()
matches = re.finditer(r'pi\.registerTool\(\{([\s\S]*?)\}\);', c)
broken = []
for m in matches:
    b = m.group(1)
...
" 2>/dev/null || true)
    if [[ -n "$broken_tools" ]]; then
        check "tool contract: $ext (missing execute for $broken_tools)" "FAIL"
        CONTRACT_ERRORS=$((CONTRACT_ERRORS + 1))
    fi
done
if [[ "$CONTRACT_ERRORS" -eq 0 ]]; then
    check "all registered tools implement execute: contract" "OK"
fi
...
echo
echo "[ Agent/Skill Catalog Health (shared) ]"
harness_run_shared_test "agent/skill catalog health (claude/agy/pi)" "$ROOT/agent/scripts/test-catalog-health.sh"

echo
echo "[ Pi Extension Unit Tests (bun) ]"
# Enforce the extension unit suite (extensions/lib/*.test.ts). Made runnable by
# extensions/bunfig.toml + lib/test-preload.ts (virtual stubs for typebox /
# @earendil-works/pi-tui); this gate catches regressions in extension logic and
# any new runtime-only import that escapes the preload. Skips gracefully if bun
# is unavailable.
harness_run_shared_test "pi extension unit tests (bun test)" "$ROOT/agent/scripts/test-pi-extensions.sh"
