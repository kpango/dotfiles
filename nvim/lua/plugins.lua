...
-- - LSP, 補完, フォーマッター／リンター (none-ls), DAP, Copilot 等の統合
-- - safe_require 関数による安全なモジュール読み込み
-- - Telescope によるファジー検索でファイルを開く機能
-- - 効率的な開発を支援する各種 UI/UX 改善プラグインの統合
-- - Ctrl+C によるコメントアウト切り替え（通常・ビジュアルモード）
-----------------------------------------------------------
...
require("lazy").setup({
	...
	------------------------------------------------------------------
	-- Plugin: nvim-lspconfig (LSP 基本設定)
	------------------------------------------------------------------
	{
	...
	------------------------------------------------------------------
	{
		"nvim-tools/none-ls.nvim", -- ※ リポジトリ名は実際のものに合わせてください
		config = function()
	...
	{
		"jay-babu/mason-none-ls.nvim", -- ※ 実際の名称に合わせてください
		dependencies = { "mason.nvim", "nvim-tools/none-ls.nvim" },
		config = function()
			safe_require("mason-none-ls").setup({
				ensure_installed = { "clang-format", "gofmt", "rustfmt", "zigfmt", "nimpretty", "black", "flake8" },
			})
		end,
	},
	...
	------------------------------------------------------------------
	{
		...
		config = function()
			...
			-- 例: C/C++ 用のデバッガー (cppdbg)
			dap.adapters.cppdbg = {
				id = "cppdbg",
				type = "executable",
				command = "path/to/cpptools/extension/debugAdapters/bin/OpenDebugAD7", -- 各自パスを調整
			}
...
vim.keymap.set("n", "<F10>", function()
	safe_require("dap").step_over()
end, { silent = true })
vim.keymap.set("n", "<F11>", function()
	safe_require("dap").step_into()
end, { silent = true })
vim.keymap.set("n", "<F12>", function()
	safe_require("dap").step_out()
end, { silent = true })
